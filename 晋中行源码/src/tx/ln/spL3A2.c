...
{ 		
	...
	while(1)
	{
		...
		**********************/	
		...
		strcpy(g_pub_tx.sub_tx_code,"L3A2");
		...
		/* 开户机构号变换 */
		strcpy( g_ln_mst.opn_br_no, brno );
...
OkExit:
    ...
    sprintf(acErrMsg,"贷款剥离转入成功！[%s]",g_pub_tx.reply);
    WRITEMSG
    set_zd_data(DC_REPLY,g_pub_tx.reply);
    return 0;
    
ErrExit:
    sprintf(acErrMsg,"贷款剥离转入失败！[%s]",g_pub_tx.reply);
    WRITEMSG
    set_zd_data(DC_REPLY,g_pub_tx.reply);
    return 1;
}
