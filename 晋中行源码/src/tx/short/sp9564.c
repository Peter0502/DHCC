...
* 修 改 人:
* 修改内容:
* 注    意: 这里依旧采用了大额的fd123域,所以这个短服务,  只是用于小额的非支付交易          中, 只是用于小额的非支付交易中,如果在支付交易中需要查询小额支付交易的            明细需要采用sp9604这个短服务 
*****************************************************/
...
int sp9564()
{
	...
	vtcp_log("sp9564 开始！[%s][%d]",__FILE__,__LINE__);
	...
	get_zd_long("0440",&lv_pkgreg.wt_date);	   /*委托日期*/	
	memcpy(lv_pkgreg.txnum,fd123.o_txnum,sizeof(lv_pkgreg.txnum)-1);
	...
	if(lv_pkgreg.txnum[0]==0x00)/***如果没有输入业务类型的话  就算了***/
	{
	...
	{
		ret = Lv_pkgreg_Sel(g_pub_tx.reply,&lv_pkgreg, "orderno='%s' and or_br_no='%s' and wt_date=%d and txnum='%s'" , \ 
		lv_pkgreg.orderno,lv_pkgreg.or_br_no,lv_pkgreg.wt_date,lv_pkgreg.txnum);
	}
