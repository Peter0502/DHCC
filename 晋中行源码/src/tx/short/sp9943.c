...
int sp9943()
{
	...
	/*核销 && ( (挂失&&未用退回) || (逾期退回) )*/
	if( mbfebillopnreg_v.billsts[0] =='*' && mbfebillopnreg_v.useind[0] == '5' )
	;
	else
	{
		if( mbfebillopnreg_v.useind[0] == '6' )
		{
			sprintf( acErrMsg, "该张汇票退回资金已提入!", mbfebillopnreg_v.billcode);
			MBFEWRITEMSG
			goto ErrExit;
		}
		else
		{
			sprintf( acErrMsg, "该张汇票资金并未逾期退回!", mbfebillopnreg_v.billcode);
			MBFEWRITEMSG
			goto ErrExit;
		}
	}
	...
	{
		sprintf( acErrMsg, "不能兑付他行出票的逾期退回资金!出票行:%s",mbfebillopnreg_v.payeropenaccbkcode);
		MBFEWRITEMSG
		goto ErrExit;
	}
