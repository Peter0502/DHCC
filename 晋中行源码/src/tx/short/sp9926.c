...
int sp9926()
{
	...
	/* 检查贴现登记薄中是否存在此贴现协议编号 */
	ret = Mo_discnt_Sel( g_pub_tx.reply, &sMoDiscnt, "(pact_no='%s' and pn_type='1') \
		or (pact_no='%s' and pn_type='3') or (pact_no='%s' and pn_type='2') ", \
		sMoDiscnt.pact_no, sMoDiscnt.pact_no, sMoDiscnt.pact_no);
	if( ret == 100 )
	{
...
/*
	...
	set_zd_data( "0700", sMoDiscnt.get_ac_type );	/* 收款账户类型 */
