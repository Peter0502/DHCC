...
{	  
	...
	*******************************/
	...
	vtcp_log("[%s,%d]交易机构=[%s]",__FILE__,__LINE__,lv_pkgreg.br_no);

	if(!memcmp(lv_pkgreg.txnum,TXNUM_TYJL,sizeof(lv_pkgreg.txnum)-1))/**YHBP添加对通用截留的处理**/
    {
    	  FILE *fpw;
        PAY_IN_AREA payin;
        ...
        vtcp_log("[%s][%d] 写入附加域文件2=[%s]",__FL__,(char *)&lv_tyjl2);
        memset(cBuff, 0 , sizeof(cBuff));
        ...
        vtcp_log("[%s][%d] 得到的附加域3=[%s]",__FILE__,__LINE__,cBuff);
        memcpy((char *)&lv_tyjl3,cBuff, sizeof(lv_tyjl3));
        vtcp_log("[%s][%d] 写入附加域文件3=[%s]",__FL__,(char *)&lv_tyjl3);
        memset(cBuff, 0 , sizeof(cBuff));
        ...
        vtcp_log("[%s][%d] 得到的附加域5=[%s]",__FILE__,__LINE__,cBuff);
        memcpy((char *)&lv_tyjl5,cBuff, sizeof(lv_tyjl5));
        vtcp_log("[%s][%d] 写入附加域文件5=[%s]",__FL__,(char *)&lv_tyjl5);
        memset(cBuff, 0 , sizeof(cBuff));
     ...
     memcmp(lv_pkgreg.txnum,TXNUM_HD  ,5)!=0 )
   {
    ...
    if(memcmp(lv_pkgreg.txnum,TXNUM_GZDJ,sizeof(lv_pkgreg.txnum)-1)==0 || \
		...
		if(ret){
			if(ret==100){
				set_zd_data("0830","");
				sprintf(acErrMsg,"没找到啊orderno=[%s]",lv_pkgreg.orderno);
				WRITEMSG
			}else{ 	 	
