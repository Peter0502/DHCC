...
int sp9793()
{
	...
	{
		sprintf( acErrMsg, "此汇票已移存资金!" );
		MBFEWRITEMSG
		goto ErrExit;
	}
	else if( mbfebillopnreg_v.billsts[0]=='*' )
	{
		sprintf( acErrMsg, "此汇票已核销,请检查核销原因!" );
		MBFEWRITEMSG
		goto ErrExit;
	}
	else
	{
		sprintf( acErrMsg, "此汇票状态不正常,不能移存,请联系中心机房![%s]", mbfebillopnreg_v.billsts );
		MBFEWRITEMSG
		goto ErrExit;
	}
	...
	{
		sprintf( acErrMsg, "非本行签发汇票,应由签发:%s行移存!", mbfebillopnreg_v.payeropenaccbkcode );
		MBFEWRITEMSG
		goto ErrExit;
	}
