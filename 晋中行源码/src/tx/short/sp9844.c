...
{ 		
	...
	vtcp_log("%s,%d[%s]",__FILE__,__LINE__,prod_no);
	...
	switch( prod_no[0] )
	{
		case '1':
			...
			*/
			...
			set_zd_data( "1034", g_dd_parm.sttl_type );
			if( g_dd_parm.cif_type[0]=='0' || g_dd_parm.cif_type[0]=='1' 
				|| g_dd_parm.cif_type[0]=='5' )
			{
				fprintf(fp,"1034|个人结算户|D|\n");
				fprintf(fp,"1034|个人储蓄户|S|\n");
			}
		...
		case '2':
			...
			set_zd_data( "103G", g_td_parm.tfr_lmt_ind );
	...
	vtcp_log("%s,%d,title=[%s],cur_no=[%s],intst_type=[%s],td_type=[%s],term=[%d]",__FILE__,__LINE__,g_td_parm.title,g_td_parm.cur_no,g_td_parm.intst_type,g_td_parm.td_type,g_td_parm.term);			
			break;
	}
