...
{ 		
	...
	if( flag[0] =='1' )
	{
	 	rate=g_ln_mst.rate;	/* 正常 */
	}
	 				
	if( flag[0] =='2' )
	{
	 	rate=g_ln_mst.exp_rate;	/* 展期 */
	}
	 
	if( flag[0] =='3')
	{
	 	rate=g_ln_mst.over_rate;	/* 逾期 */
	}	
	 
	if( flag[0] =='4')
	{
	 	rate=g_ln_mst.fine_rate;	/* 罚息 */
	}	 
	
	/* 输出变量 */
	sprintf(acErrMsg,"进入输出变量[%s]",g_ln_mst.name);
	WRITEMSG	
	set_zd_double( "0840", rate );			/* 利率 */			
