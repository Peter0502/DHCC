...
{ 		
	...
	if(i==3||i==5||i==7)							/*输入科目*/
	{
		...
		{
			set_zd_data("121B","借");			
			set_zd_double("1213",g_gl_mst.dr_bal);			
		}
		else if(g_com_item.dc_ind[0]=='2')
		{
			set_zd_data("121B","贷");			
			set_zd_double("1213",g_gl_mst.cr_bal);			
		}
		...
		set_zd_data("1212",g_com_item.acc_name);       /*户名*/
		set_zd_data("1214",g_gl_mst.cur_no);           /*币种*/
	}
	else												/*输入内部帐号*/
	{
		/*根据帐号找账户ID和序号 */
		...
		/* 输出变量 */
		set_zd_data("1212",g_in_mst.name);              /*户名*/
		set_zd_data("1214",g_in_parm.cur_no);           /*币种*/
		set_zd_double("1213",g_in_mst.bal);             /*余额*/
	}
