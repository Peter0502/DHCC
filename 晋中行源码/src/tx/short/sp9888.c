...
{ 		
	...
	vtcp_log( "(prdt_no in(select prdt_no from td_parm where %s) or prdt_no in(select prdt_no from td_parm where %s)) and mdm_code2='%s' and mdm_code3='%s' ",
		wherelist,wherelist,NULL_MDM,NULL_MDM);
	/***介质列表***/
	ret=Mdm_prdt_rel_Dec_Sel( g_pub_tx.reply,
		"(prdt_no in(select prdt_no from td_parm where %s) or prdt_no in(select prdt_no from td_parm where %s)) and mdm_code2='%s' and mdm_code3='%s' order by mdm_code1",
		wherelist,wherelist,NULL_MDM,NULL_MDM);
	if( ret ) goto ErrExit;
