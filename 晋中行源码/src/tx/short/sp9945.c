...
int sp9945()
{
	...
	ret = Mo_ibl_ret_Sel(g_pub_tx.reply, &sMo_ibl_ret, "pact_no='%s' and \
			sts='1'", sMo_ibl_c.pact_no);
	if (ret != 0 && ret != 100)
	{
		sprintf(acErrMsg, "查询拆入还款登记簿错误!! [%d]", ret);
		WRITEMSG
		goto ErrExit;
	}
	else if (ret == 100)
	{
		strcpy(g_pub_tx.reply, "B251");
		goto ErrExit;
	}
	...
	set_zd_double("0420", sMo_ibl_ret.ret_amt);
	set_zd_double("0430", sMo_ibl_ret.ret_rate);
