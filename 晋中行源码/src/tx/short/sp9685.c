...
OkExit:
	...
	sprintf(acErrMsg,"部提计息成功![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 0;
ErrExit:
	sprintf(acErrMsg,"部提计息失败![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 1;
}
...
int sub_wrt_file()
{
	...
	if(g_pub_tx.prdt_code[0]=='1')
	{
		...
		{
		 fprintf(fp,"LXQD%s|%s|%.2lf|%s|%ld|%s|%.2lf|%.2lf|%.2lf|20|%.2lf|%.2lf|%.2lf|%.2lf|%s|%s|%d|%s|%d|%s|%s|%d|%ld|%d|%.2lf|%s|\n",
	   ...
	   "",g_mdm_ac_rel.prt_line,g_pub_tx.brf,"",0,0,0,g_dd_mst.rate,"");
	   }
	   else
	   {
		 fprintf(fp,"LXQD%s|%s|%.2lf|%s|%ld|%s|%.2lf|%.2lf|%.2lf|20|%.2lf|%.2lf|%.2lf|%.2lf|%s|%s|%d|%s|%d|%s|%s|%d|%ld|%d|%.2lf|%s|\n",
	...
	else if(g_pub_tx.prdt_code[0]=='2')
	{
	   ...
	   g_pub_tx.tx_amt1+g_pub_intst.dealval+g_pub_intst.keepval-g_pub_intst.val,
	   	...
	   	g_td_mst.book_line,g_td_mst.mtr_date,0,g_td_mst.rate,"");
		}
		else
		{
		 fprintf(fp,"LXQD%s|%s|%.2lf|%s|%ld|%s|%.2lf|%.2lf|%.2lf|20|%.2lf|%.2lf|%.2lf|%.2lf|%s|%s|%d|%s|%d|%s|%s|%d|%ld|%d|%.2lf|%s|\n",
