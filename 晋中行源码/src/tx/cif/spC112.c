...
int spC112()
{
		...
		strcpy(sCif_family_inf.br_no,g_pub_tx.tx_br_no);
    strcpy(sCif_family_inf.tel,g_pub_tx.tel);
		
		ret=Cif_basic_inf_Sel( g_pub_tx.reply,&sCif_basic_inf,
							"cif_basic_inf.cif_no=%ld",sCif_family_inf.cif_no);
		if( ret==100)
		{
	...
	sCif_family_inf.tx_date = g_pub_tx.tx_date;
	sCif_family_inf.up_date = g_pub_tx.tx_date;
	
	ret=sql_count("cif_family_inf","cif_no=%ld and manage_no = %ld",sCif_family_inf.cif_no,sCif_family_inf.manage_no);
	if(ret<0)
	{
	...
	{
		sCif_family_inf.family_no=1;

		/* 登记客户家庭信息表 */
		iRet = Cif_family_inf_Ins(sCif_family_inf,g_pub_tx.reply);
		if(iRet)
		{
			sprintf(acErrMsg,"登记客户家庭信息表失败！");
	...
	{
		ret=sql_max_int("cif_family_inf","family_no",\
						"cif_no=%ld and manage_no=%ld",sCif_family_inf.cif_no,sCif_family_inf.manage_no);
		if(ret<0)
		{
		...
		{
			sCif_family_inf.family_no=ret+1;
		...
		set_zd_int("0350",sCif_family_inf.family_no);

	/*登记交易流水*/
	g_pub_tx.ac_id = sCif_family_inf.cif_no;
	strcpy(g_pub_tx.brf,"客户家庭信息录入");
...
OkExit:
	...
	sprintf(acErrMsg,"客户家庭工作信息录入成功!!%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data(DC_REPLY,g_pub_tx.reply);
	return 0;
ErrExit:
    sprintf(acErrMsg,"客户工作家庭录入失败!!%s]",g_pub_tx.reply);
    WRITEMSG
    set_zd_data(DC_REPLY,g_pub_tx.reply);
    return 1;		
}
