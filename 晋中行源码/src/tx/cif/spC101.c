...
{ 	
	...
	vtcp_log(" cif_basic_inf.name = [%s]\n",cif_basic_inf.name);
	vtcp_log(" cif_addr_inf.addr = [%s]\n",cif_addr_inf.addr);

        
        /*test*/
        double tx_amt = -1250.55;
        double tx_amt1 = 0.0;
  
        set_zd_double("0390",tx_amt);
        get_zd_double("0390",&tx_amt1);

        sprintf(acErrMsg,"测试金额:[%f]",tx_amt1);
        WRITEMSG
        /*end*/

	/***增加对客户名和客户地址字符的检查 add by wanglei 20060926***/
	/****
	if(iCharacterType(cif_basic_inf.name,sizeof(cif_basic_inf.name),'g'))
	{
		sprintf(acErrMsg,"客户名含有非法字符![%s]",g_pub_tx.reply);
		WRITEMSG
		strcpy(g_pub_tx.reply,"C127");
		goto ErrExit;
	}***20061112***/
	...
	*/
	vtcp_log("[%s][%d] cif_addr_inf.addr = [%s]\n",__FILE__,__LINE__,cif_addr_inf.addr);
	...
	if (atoi(cif_basic_inf.type)!=1)
	{
		...
		ret=Cif_basic_inf_Sel(g_pub_tx.reply,&cif_basic_inf,"name='%s'and type!='%s' and cif_no<60000000   and sts = '1'",cif_basic_inf.name,"1");
		if (ret==0)
		{
	...
	if (cif_basic_inf.type[0]=='1'||cif_basic_inf.type[0]=='5')
	{
		...
		if ((!strncmp(cif_email_inf.email,"13",2)||!strncmp(cif_email_inf.email,"15",2)||!strncmp(cif_email_inf.email,"18",2))
			&&strlen(cif_email_inf.email)==11)
