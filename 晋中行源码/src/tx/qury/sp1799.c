	...
	while(1)
	{
		...
		if(((ttlnum/10+1)==atoi(current_page))&&(ttlnum%10!=0)||((ttlnum%10==0)&&ttlnum/10==atoi(current_page)))
		{
		  ...
		  get_opn_branch(lv_pkgreg.pay_opn_br_no,br_name1);
			...
			else if(memcmp(vdd_mst_hst.tx_code,"7021",4)==0||memcmp(vdd_mst_hst.tx_code,"6236",4)==0||memcmp(vdd_mst_hst.tx_code,"7023",4)==0||memcmp(vdd_mst_hst.tx_code,"7022",4)==0||memcmp(vdd_mst_hst.tx_code,"7023",4)==0||memcmp(vdd_mst_hst.tx_code,"6247",4)==0)
			{
				ret=Chnl_tx_detl_Sel(g_pub_tx.reply,&chnl_tx_detl,"tx_date=%ld and trace_no=%ld and ac_no='%s' and tx_amt=%.2f",vdd_mst_hst.tx_date,vdd_mst_hst.trace_no,tmpac_no,vdd_mst_hst.tx_amt);
				if(ret && ret != 100)
				{
					vtcp_log( "查询支付信息失败");
					goto ErrExit;
				...
				get_branch(vdd_mst_hst.opn_br_no,br_name);
				memset(br_name1,0x00,sizeof(br_name1));
				get_opn_branch(chnl_tx_detl.or_br_no,br_name1);
				fprintf( fp,"%s|%s|%s|%s|%ld|%s|%ld|%s|%.2lf|%.2lf|%s|%s|%s|%s|%s|\n",\
				tmpac_no,f_dd_mst.name,br_name,ac_sts,vdd_mst_hst.tx_date,vdd_mst_hst.brf,vdd_mst_hst.trace_no,"无",\
				vdd_mst_hst.tx_amt,vdd_mst_hst.bal,"贷","RMB",chnl_tx_detl.df_ac_no,chnl_tx_detl.df_name,br_name1); 
			}
		  	else
		  	{
