...
sp1904()
{
	...
	while(1)
	{
		...
		/** 限制只能查自己支行的  别家的直接跳过 modify by wudawei 20140711
		g_reply_int=Mdm_ac_rel_Sel(g_pub_tx.reply,&g_mdm_ac_rel, \
									"ac_id=%ld and note_sts not in ('2','3','4') ",g_prdt_ac_id.ac_id);
		modify by wudawei 20140711 **/							
		g_reply_int=Mdm_ac_rel_Sel(g_pub_tx.reply,&g_mdm_ac_rel, \
									"ac_id=%ld and note_sts not in ('2','3','4') and opn_br_no='%s'",g_prdt_ac_id.ac_id, g_pub_tx.tx_br_no);
		if(g_reply_int==100)
		{
