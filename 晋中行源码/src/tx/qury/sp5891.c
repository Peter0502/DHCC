...
/*	memset(tx_date,0,sizeof(tx_date)); */ 
  ...
  vtcp_log("[%s][%d]cTdtlid=[%s]",__FILE__,__LINE__,cTdtlid);
	vtcp_log("sp5891 开始！[%s][%d]",__FILE__,__LINE__);
	ret=Hv_zf_Sel(g_pub_tx.reply,&f_hv_zf,"cmtno='%s' and tx_date=%d and lw_ind='%s' and or_br_no='%s' and orderno='%s'",cTdtlid,apatoi(tx_date,8),lw_ind,or_br_no,orderno);
	   vtcp_log("[%s][%d]cTdtlid=[%s]",__FILE__,__LINE__,cTdtlid);	
	   vtcp_log("sp5891 开始！[%s][%d]",__FILE__,__LINE__);
	   ...
	   else if ( ret )
	   {
    	...
    	ret=Hv_zf_Dec_Sel(g_pub_tx.reply,"cmtno='%s' and tx_date=%d and lw_ind='%s' and or_br_no='%s' and orderno='%s'",cTdtlid,apatoi(tx_date,8),lw_ind,or_br_no,orderno);
	    vtcp_log("查询成功！  [%s][%d]",__FILE__,__LINE__);
	    vtcp_log("查询成功！  [%s][%d]",__FILE__,__LINE__);
