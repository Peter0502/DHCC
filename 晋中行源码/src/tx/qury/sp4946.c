...
int sp4946()
{
	...
	get_zd_data("0380",f_mo_discnt.rpact_no );
	...
	/**组成查询条件**/
	pub_base_strpack( f_mo_discnt.rpact_no );
	if( strlen(f_mo_discnt.rpact_no) )
	{
		sprintf( tmpstr," rpact_no='%s' and",f_mo_discnt.rpact_no );
	...
	strcat( wherelist, " pn_type in ('6','7') and " );
	...
	strcpy( fieldlist,"rpact_no,ac_id,ac_seqn,br_no,bkrl_code,pn_type,tranx_bal,pn_rate,pn_date,local_ind,pn_int,pn_name,pay_ac_type,get_ac_type,tpay_ac_type,remitter,rem_ac_no,payer_brno_name,payee,payee_ac_no,payee_brno_name,par_amt,not_matr_date,note_type,pnote_no,way_days,wrk_date,trace_no,sts");
	...
	while(1)
	{
		...
		if( !ttlnum )
		{
			...
			/**显示列标题**/
			fprintf( fp,"~@再贴现协议编号|再贴现账号|@机构编码|@产品编号|@贴现类型|$贴现账户余额|贴现率|贴现日|@异地同城标志|$贴现利息|@贴现申请人户名|@贴现还款记账类型|@收款帐户类型|@付款帐户类型|@出票人全称|@出票人帐号|@付款行全称|@收款人全称|@收款人帐号|@收款人开户行全称|$汇票金额|汇票到期日|@票据类型|@票据号|在途天数|流水日期|主机流水号|@状态|\n" );
		}
