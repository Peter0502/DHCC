...
spXEJZ()
{
	...
	vtcp_log("%s,%d cur_no=======[%s]",__FILE__,__LINE__,g_pub_tx.cur_no);
	...
	/**网银大小额需要行里要求修改摘要 20130809 hzh**/
	memset(netbank_flag,0,sizeof(netbank_flag));
	get_zd_data("0810",netbank_flag);
	if(strcmp(netbank_flag,"netbank",7) == 0)
	{
	get_zd_data("0820",g_pub_tx.brf);
	}
