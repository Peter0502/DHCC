...
spHPDF()
{
	...
	vtcp_log("%s,%d进入大额来帐帐务处理 =======[%s]",__FILE__,__LINE__,g_pub_tx.cur_no);
	vtcp_log("%s,%d进入大额来帐帐务处理 币种=======[%s]",__FILE__,__LINE__,g_pub_tx.cur_no);
	vtcp_log("%s,%d进入大额来帐帐务处理 币种=======[%s]",__FILE__,__LINE__,g_pub_tx.cur_no);
	...
	/*** 初始化全局变量 ***/
	vtcp_log("%s,%d进入HPDF1  币种=======[%s]",__FILE__,__LINE__,g_pub_tx.cur_no);
	pub_base_sysinit();
	vtcp_log("%s,%d进入HPDF2  币种=======[%s]",__FILE__,__LINE__,g_pub_tx.cur_no);

	get_zd_data("0680",trance_type);
	get_zd_data("0300",g_pub_tx.ac_no);
	get_zd_data("0210",g_pub_tx.cur_no);
	vtcp_log("%s,%d 币种=======[%s]",__FILE__,__LINE__,g_pub_tx.cur_no);
	vtcp_log("%s,%d 帐号类型===[%s]",__FILE__,__LINE__,trance_type);
