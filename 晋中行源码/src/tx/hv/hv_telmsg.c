...
hv_telmsg()
{
  ...
  char ptaddr[16];
	...
	/**查询大额来账未发送卡系统记录**/
	iRc = Hv_zf_Dec_Sel(g_pub_tx.reply," tx_date=%ld and cmtno in ('100','108') and substr(cash_ac_no,0,6) in ('%s','%s') and lw_ind='2' and tx_chrg_ind is NULL order by tx_time ",com_sys_parm.sys_date, CARDHEADCODE, ICCARDHEADCODE);
	if(iRc==100 || iRc==1403)
	{
		...
		{
			vtcp_log("[%s][%d],有大额来账数据,需进行相关处理!",__FILE__,__LINE__);
			iCount=0;
			while(1)
			{
			...
			if(ret==100 || ret==1403)
		      ...
		      continue;
					...
					{
						vtcp_log("[%s],[%d] or_br_no=[%s], wt_date=[%d], orderno=[%s], cmtno=[%s]",__FILE__,__LINE__,hv_zf.or_br_no,hv_zf.wt_date,hv_zf.orderno,hv_zf.cmtno);
					  memset(sendbuf,0x00,sizeof(sendbuf));
					  sprintf(sendbuf,"00866920%-32s%-15.0f%-32s%-3s",hv_zf.cash_ac_no,hv_zf.tx_amt*100,hv_zf.pay_ac_no,hv_zf.cmtno); /**设定交易代码为6920,报文体长度为86**/
					  /**建立连接**/
           ...
           if(memcmp(buf,"0000",4)==0)
             {
               vtcp_log("%s,%d,收到正确应答[%s]!",__FILE__,__LINE__,buf);
	             ...
	             if(iRet)
	              {
	            	  vtcp_log("%s,%d,打开事务失败[%d]!",__FILE__,__LINE__,iRet);
	            	  continue;
	              }
	             memset(sqllist,0x00,sizeof(sqllist));
	             sprintf(sqllist,"update hv_zf set tx_chrg_ind = '9' where or_br_no='%s' and wt_date=%ld and orderno='%s' and cmtno='%s' ",hv_zf.or_br_no,hv_zf.wt_date,hv_zf.orderno,hv_zf.cmtno);
	             iRet = sql_execute(sqllist);
	             if (iRet) {
	            	  vtcp_log("%s,%d,修改大额来账发送卡系统标志出错[%d]!",__FILE__,__LINE__,iRet);
	            	  continue;
	              }
	             sql_commit();           /*--提交事务--*/
	             iCount++;
	             vtcp_log("%s,%d,更新大额来账第[%d]条记录成功!",__FILE__,__LINE__,iCount);
             }
          else
      	    {
			        FileSock_close(iSockId);
