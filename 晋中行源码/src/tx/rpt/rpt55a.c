...
rpt55a()
{
  ...
  int ret13=0,ret14=0;/**add zgf 20130926***/
	...
	struct zjgl_mst_c tmp_zjgl1;
        memset( pre_br_no, 0x00, sizeof( pre_br_no ) );
          
    vtcp_log("[%s][%d]开始打印支行资金头寸汇总!\n",__FILE__,__LINE__);
	...
	strcpy(vpfm, "rpt55a");
        ...
        ret = Com_branch_Dec_Sel(g_pub_tx.reply,"1=1 and br_no  in ('10101') and wrk_sts!='*'  order by br_no");
	if(ret)
	{
	...
	while(1)
	{
		memset(&sCom_branch, 0x0, sizeof(struct com_branch_c));
          ...
          /***zgf 20130926 原来注释掉**
       		...
       		*/
       		/***mob zgf 20130926**/
        	ret1 = sql_sum_double("gl_sub","cr_bal",&tmp_amt1,"acc_hrt in ('20100','20200','20300','20500','21100','21500','21700','21800','22100','22200','24300','25100','22300','22500','22600','23400','23500','23600','25600') and br_no in('10101','11901') order by br_no");
       		/***mob end zgf****/
       		if (ret1)
		{
		...
		ret2=sql_sum_double("gl_sub","dr_bal",&tmp_amt2,"acc_hrt in ('12200','12300','12400','12500','12600','12700','12800','12900','13000') and br_no in('10101','11901') order by br_no");
       		if (ret2)
		{
		 ...
		 ret3 = sql_sum_double("gl_sub","dr_bal",&tmp_amt3,"acc_hrt in('11500','11400') and br_no in('10101','11901') order by br_no");
       		if (ret3)
		{
		...
		ret4 = sql_sum_double("zjgl_mst","dr_bal",&tmp_amt4,"acc_no in ('4070201') and opn_br_no in('10101','11901') order by opn_br_no");
       		if (ret4)
		{
		...
		ret5 = sql_sum_double("zjgl_mst","dr_bal",&tmp_amt5,"acc_no in ('4070202') and opn_br_no in('10101','11901') order by opn_br_no");
       		if (ret5)
		{
		...
		*/
		...
		memset(&tmp_zjgl1,'\0',sizeof(tmp_zjgl1));
		ret6 = Zjgl_mst_Sel(g_pub_tx.reply ,&tmp_zjgl,"acc_no ='4070203' and opn_br_no in('10101') and cur_no='01'");
       		if (ret6)
		{
		...
		ret6 = Zjgl_mst_Sel(g_pub_tx.reply ,&tmp_zjgl1,"acc_no ='4070203' and opn_br_no in('11901') and cur_no='01'");
       		if (ret6)
		{
			sprintf(acErrMsg, "统计资金管理登记簿清算资金错误!! [%d]", ret6);
			WRITEMSG
		...
		tmp_amt6=(tmp_zjgl.dr_bal-tmp_zjgl.cr_bal) + (tmp_zjgl1.dr_bal-tmp_zjgl1.cr_bal);
...
**/
		ret7 = sql_sum_double("ass_mst","bal",&tmp_amt7,"sign='1' and br_no in('10101','11901') order by br_no");
       		if (ret7)
		{
			sprintf(acErrMsg, "统计资金管理登记簿拆借金额错误!! [%d]", ret7);
			WRITEMSG
	...
	*/
		ret8 = sql_sum_double("ass_mst","bal",&tmp_amt8,"sign='2' and br_no in('10101','11901') order by br_no");
       		if (ret8)
		{
		...
		ret10 = sql_sum_double("gl_sub","cr_bal",&tmp_amt10,"acc_hrt in ('25100') and br_no in('10101','11901') order by br_no");
       		if (ret10)
		{
		 ...
		 ret11 = sql_sum_double("gl_sub","dr_bal",&tmp_amt11,"acc_hrt='10100' and br_no in('10101','11901') order by br_no");
       		if (ret11)
		{
    ...
    /***add zgf 20130926 增加一般存款tmp_amt13  **/
  	ret13 = sql_sum_double("gl_sub","cr_bal",&tmp_amt13,"acc_hrt in ('20100','20200','20300','20500','21100','21500','21700','21800','22500','22600','23600','24300','25100','25600') and br_no in('10101','11901') order by br_no");
 		if (ret13)
		{
  	...
  	ret14 = sql_sum_double("gl_sub","cr_bal",&tmp_amt14,"acc_hrt in ('20100','20200','20300','20500','21100','21500','21700','21800','22500','22600','23600','25100','25600') and br_no in('10101','11901') order by br_no");		
 		if (ret14)
		{
	   ...
	   " br_no in('10101','11901') and acc_hrt='40100' ");	   
			/** 402 代理证券业务轧差后贷方余额 **/
	   ret = sql_sum_double( "gl_sub","cr_bal-dr_bal",&d402_bal,\
	   " br_no in('10101','11901') and acc_hrt='40200' ");	   
	      /** 403 其他代理业务轧差后贷方余额 **/
	   ret = sql_sum_double( "gl_sub","cr_bal-dr_bal",&d403_bal,\
	   " br_no in('10101','11901') and acc_hrt='40300' ");	   
	      /** 406 委托业务轧差后贷方余额 **/
	   ret = sql_sum_double( "gl_sub","cr_bal-dr_bal",&d406_bal,\
	   " br_no in('10101','11901') and acc_hrt='40600' ");   
	   tmp_amt14+=d401_bal+d402_bal+d403_bal+d406_bal;
	   /** wudawei 20130927 end **/
		   
		   	
    ret121=sql_sum_double("gl_sub","cr_bal",&tmp_amt121,"acc_hrt in('50100','50200','51100','51200','51300','51400','51500','52100','52200','52300','52700','52900','53000','53100','53500','53400','53500','53600','55000','56000') and br_no in('10101','11901') order by br_no");
    if (ret121)
		{
		...
		ret122=sql_sum_double("gl_sub","dr_bal",&tmp_amt122,"acc_hrt in('50100','50200','51100','51200','51300','51400','51500','52100','52200','52300','52700','52900','53000','53100','53500','53400','53500','53600','55000','56000') and br_no in('10101','11901') order by br_no");
       		if (ret122)
		{
		...
		ret123=sql_sum_double("gl_sub","dr_bal",&tmp_amt123,"acc_hrt='53300' and br_no in('10101','11901') order by br_no");
       		if (ret123)
		{
...
char str[100];
{
	...
	switch( bli ) 
	{
		case 'A':
			...
			sprintf(vstr, "%s 汇总（%s）", sCom_branch.br_name, sCom_branch.br_no );
