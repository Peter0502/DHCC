...
int get_ratio_lnjxkkhd(int bli,int bll,char str[100]);
...
rpt041()
{
	...
	pub_base_sysinit();
/*
	strcpy( filename , "BJlnjxkkhd" );	
	strcpy(vpfm, "lnjxkkhd" );
*/	
	strcpy( filename , "RPT041");
	strcpy(vpfm, "RPT041");
	/* 本金积数结息清单 */
	/* 条件:tx_date为当日,tx_code='G083',增减标志为减少 */
	memset( &tmpstr,0x0,strlen(tmpstr));	
	memset( &wherelist,0x0,strlen(wherelist));
	sprintf( tmpstr," tx_code='G083' and tx_date=%ld and add_ind='0' and " ,qry_date );
	...
	while(1)
	{
		...
		{	
			...
			if( strcmp( s_ln_mst_hst.opn_br_no , old_br_no ) )
			{
				...
				{
					TRACE
					/** if( line_file==3 ) del by martin 070520**/
					if( line_file==2 ) 
					{ 
						/* 打页尾 */
						ret = Make_yeb_sub("CC");
				...
				/*line_file=0;*/
				line_file=-1;/**by liuyue 20061220,如果是换机构了,本次循环的记录仍旧是上个机构的，但是
						程序顺序执行了 下面的line_file++操作，这就导致了某个机构的第一页少打印了一个表格**/
				page=1;
...
OkExit:
	...
	sprintf(acErrMsg,"生成贷款结息清单2成功!!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 0;
ErrExit:
	sql_rollback();	/*--事务回滚--*/
	sprintf(acErrMsg,"生成贷款结息清单2失败!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 1;
}
...
{	
	ret = pub_rpt_read_pfm_qd(fp,&line,vpfm,"0001",vrec,&opncnt,get_ratio_lnjxkkhd);
	ERR_DEAL
...
/* 赋值函数 */
int get_ratio_lnjxkkhd( bli,bll,str )
int bli,bll;
char str[100];
{ 
	char vhm[101]; 
	...
	{ 
		...
		case 'J': /* 还息金额(大写) */
			sprintf(fmt,"%%-%ds",bll);
			sprintf(str, fmt, pub_rpt_pcask(out_amt+in_amt+cmpd_amt));
			break;
		case 'K': /* 还息金额(小写) */
			/* 加逗号 */
			sprintf( amt_tmp, "%.2lf" , out_amt+in_amt+cmpd_amt ); 
		...
		case 'L': /* 利息金额 */
			/* 加逗号 */
			sprintf( amt_tmp, "%.2lf" , out_amt+in_amt ); 
			pub_rpt_flttomon( amt_tmp,amt_tmp );
		...
		case 'P': /* 尚欠利息 */
			/* 加逗号 */
			sprintf( amt_tmp, "%.2lf" , s_ln_mst.in_lo_intst+ s_ln_mst.out_lo_intst+ s_ln_mst.cmpd_lo_intst ); 
			pub_rpt_flttomon( amt_tmp,amt_tmp );
