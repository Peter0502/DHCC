...
rpt972()
{
	...
	pub_base_sysinit();
/*
	strcpy( filename , "BJnbjxqd" );	
	strcpy(vpfm, "nbjxqd" );
*/	
	strcpy( filename , "RPT972");
	strcpy(vpfm, "RPT972");
	ret = Intst_hst_Dec_Sel(g_pub_tx.reply,"ac_ind='2' and pt_ind='1' and intst_date=%ld order by br_no ",g_pub_tx.tx_date );
	ERR_DEAL
	ttlnum=0;
	while(1)
	{
...
OkExit:
	...
	sprintf(acErrMsg,"生成内部计息交易清单成功!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 0;
ErrExit:
	sql_rollback();	/*--事务回滚--*/
	sprintf(acErrMsg,"生成内部计息清单失败!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 1;
}
...
{ 
	...
	{ 
		...
		case 'E': /* 账     号 */
			...
			break; 
		case 'M': /*户名*/
			ret=In_mst_Sel(g_pub_tx.reply,&g_in_mst,"ac_id=%ld ",s_intst_hst.ac_id);
			if( ret )
			{
				sprintf(acErrMsg, "取户名错误! [%d][%d]", s_intst_hst.ac_id,ret);
				WRITEMSG
				goto ErrExit;
			}
			sprintf(fmt, "%%-%ds", bll);
			sprintf(str, fmt , g_in_mst.name); 
			break; 
		case 'O': /* 结息前金额 */
		...
		case 'K': * 结息日期 *
			sprintf(fmt,"%%%dld",bll);	
			sprintf( str, fmt, s_intst_hst.intst_date ); 
			break;
		****/
		default : 
			memset( str,' ',bll ); 
