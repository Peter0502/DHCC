...
int get_ratio_lndbloqd(int bli,int bll,char str[100]);
...
rpt049()
{
	...
	strcpy( filename , "RPT049" );	
	strcpy(vpfm, "RPT049" );
		
	/**组成查询**/	
	strcpy(wherelist," (in_lo_intst>0.00 or out_lo_intst>0.00 or cmpd_lo_intst>0.00) and repay_type not in ('3','4') and prdt_no in ('351','352','353' ) and 1=1 ORDER BY opn_br_no,prdt_no,ac_id");
...
{	
	ret=pub_rpt_read_pfm_qd(fp,&line,vpfm,"0001",vrec,&opncnt,get_ratio_lndbloqd);
	ERR_DEAL
...
/* 赋值函数 */
int get_ratio_lndbloqd( bli,bll,str )
int bli,bll;
char str[100];
{ 
	char vhm[101]; 
