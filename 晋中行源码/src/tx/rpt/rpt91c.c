...
{ 
	...
	{ 
		...
		case 'E': /* 季初借余额 */
			...
			sprintf( amt_tmp, "%.2lf" , s_gl_sub.qdr_bal + gl_sub_low.qdr_bal); 
			if( !pub_base_CompDblVal(atof(amt_tmp),0.00) )
			{
				memset(str,' ',bll );
				break;
		...
		case 'F': /* 季初贷余额 */
			...
			sprintf( amt_tmp, "%.2lf" , s_gl_sub.qcr_bal + gl_sub_low.qcr_bal); 
			if( !pub_base_CompDblVal(atof(amt_tmp),0.00) )
			{
				memset(str,' ',bll );
				break;
			...
			break;
		case 'G': /* 本季借发生额 */
			sprintf( amt_tmp, "%.2lf" , s_gl_sub.qdr_amt  + gl_sub_low.qdr_amt); 
			if( !pub_base_CompDblVal(atof(amt_tmp),0.00) )
			{
				memset(str,' ',bll );
				break;
			...
			break;
		case 'H': /* 本季贷发生额 */
			sprintf( amt_tmp, "%.2lf" , s_gl_sub.qcr_amt  + gl_sub_low.qcr_amt); 
			if( !pub_base_CompDblVal(atof(amt_tmp),0.00) )
			{
				memset(str,' ',bll );
				break;
...
/*		case 'Y':  汇总 
		...
		case 'K': /* 季初借余额 */
			...
			if( flag==0 )
			{
				ret = sql_sum_double( "gl_sub","qdr_bal",&tmp_amt,"acc_hrt[1]='%c' and br_no in('10101','11901') and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) ", acc_type[0] );
			  /*if( acc_type[0]=='4' )
			  {
			...
			{
				ret = sql_sum_double( "gl_sub","qdr_bal",&tmp_amt,"acc_hrt[1]>='1' and br_no in('10101','11901') and acc_hrt[1]<='5' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) " );
			/*  ret = sql_sum_double( "gl_sub","qdr_bal",&tmp_amt1,"acc_hrt='40602' and br_no='%s' ",s_gl_sub.br_no);
				tmp_amt+=tmp_amt1;*/
			}
			ERR_DEAL
		...
		case 'L': /* 季初贷余额 */
			if( flag==0 )
			{
			  ...
			  ret = sql_sum_double( "gl_sub","qcr_bal",&tmp_amt,"substr(acc_hrt,1,1)='%c' and br_no in('10101','11901') and acc_hrt in ( select acc_hrt from com_item where acc_lvl='1' )  ", acc_type[0] );
			 /* if(acc_type[0]=='4')
			  {
			...
			else 
			{
			 ...
			 ret = sql_sum_double( "gl_sub","qcr_bal",&tmp_amt,"substr(acc_hrt,1,1)>='1' and br_no in('10101','11901') and substr(acc_hrt,1,1)<='5' and acc_hrt in ( select acc_hrt from com_item where acc_lvl='1' ) "  );
			 /* ret = sql_sum_double( "gl_sub","qcr_bal",&tmp_amt1,"acc_hrt='40601' and br_no='%s' ",s_gl_sub.br_no);
				tmp_amt+=tmp_amt1;*/
			}
			ERR_DEAL
		...
		case 'O': /* 本季借发生额 */
			if( flag==0  )
			{
				ret = sql_sum_double( "gl_sub","qdr_amt",&tmp_amt,"acc_hrt[1]='%c' and br_no in('10101','11901') and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) ", acc_type[0] );
			}
			else
			{
				ret = sql_sum_double( "gl_sub","qdr_amt",&tmp_amt,"acc_hrt[1]>='1' and br_no in('10101','11901') and acc_hrt[1]<='5' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) "  );
			}
			ERR_DEAL
		...
		case 'P': /* 本季贷发生额 */
			if( flag==0 )
			{
				ret = sql_sum_double( "gl_sub","qcr_amt",&tmp_amt,"acc_hrt[1]='%c' and br_no  in('10101','11901') and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' )", acc_type[0]);
			}
			else
			{
				ret = sql_sum_double( "gl_sub","qcr_amt",&tmp_amt,"acc_hrt[1]>='1' and br_no  in('10101','11901') and acc_hrt[1]<='5' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) " );
			}
			ERR_DEAL
		...
		case 'V': /* 季末贷余额 */
			...
			break;
		case 'Y': /* 季初余额 */
			sprintf( amt_tmp, "%.2lf" , s_gl_sub.qdr_bal+s_gl_sub.qcr_bal + gl_sub_low.qdr_bal  + gl_sub_low.qcr_bal); 
			if( !pub_base_CompDblVal(atof(amt_tmp),0.00) )
			{
				memset(str,' ',bll );
				break;
