...
#include "ln_parm_c.h"                  /*增加“科目号”，zhangpeng 20131126*/
...
struct ln_parm_c s_ln_parm;							/*增加“科目号”，zhangpeng 20131126*/
...
{ 
	...
	{ 
		...
		/**借据号、产品名称和复利息金额注释掉。zhangpeng 20131126 start**
		case 'E': // 借据号 
			sprintf(fmt,"%%-%ds",bll); 
			sprintf(str, fmt, s_ln_mst.pact_no); 
			break; 
		*借据号、产品名称和复利息金额注释掉。zhangpeng 20131126 end***/
		
		/**增加本金、利息积数、科目号、起始日期、终止日期、天数、利率。zhangpeng 20131126 start***/
		case 'E': /*本金*/
			/* 加逗号 */
		...
		case 'W': /*利率*/
			...
			sprintf(str, fmt, amt_tmp); 
			break;
		/**增加本金、利息积数、科目号、起始日期、终止日期、天数、利率。zhangpeng 20131126 end***/
