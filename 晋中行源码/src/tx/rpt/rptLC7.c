...
static long ldate = 0;
static long tmpdate = 0;
static int season = 0;


int get_ratio_lcjb(int bli,int bll,char str[200]);
...
int rptLC7()
{
  ...
  memset(cStr,0x00,sizeof(cStr));
	...
	pub_base_deadlineM(g_pub_tx.tx_date,-3,&ldate);
	/**组成查询**/
...
OkExit:
        ...
        sprintf(acErrMsg,"生成理财产品季报表成功!!!![%s]",g_pub_tx.reply);
        WRITEMSG
...
ErrExit:
        ...
        sprintf(acErrMsg,"生成理财产品季报表失败!!!![%s]",g_pub_tx.reply);
        WRITEMSG
        set_zd_data("0120",g_pub_tx.reply);
        return 1;
...
{
        ret=pub_rpt_read_pfm_qd(fp,&line,vpfm,"0001",vrec,&opncnt,get_ratio_lcjb);
        ERR_DEAL
...
/* 赋值函数 */
int get_ratio_lcjb( bli,bll,str )
int bli,bll;
char str[200];
{
	 char fmt[10];
...
/100 , sCom_sys_parm.lst_date%100 );
          ...
          season = (int )tmpdate%10000/100 /4 +1;
		      ...
		      sprintf( t_str , "%4d年%2d季度" , g_pub_tx.tx_date/10000 , season  );
			    sprintf(fmt, "%%-%ds", bll);
			    sprintf(str, fmt, t_str);
			    break;
       ...
       break;
		case 'G':/*募集笔数*/
			iCountA  = sql_count("fn_reg","   tx_br_no='%s' and sts not in ('3','*','0') and pay_date<=%ld and pay_date>%ld %s",sCom_branch.br_no,g_pub_tx.tx_date,ldate,cStr);
		  iCountA3+=iCountA;
      ...
      sprintf(str, fmt, t_str);
			break;
		case 'H':/*募集总金额*/
			ret = sql_sum_double("fn_reg","tx_amt",&dAmtA,"   tx_br_no ='%s' and sts not in ('3','*','0') and pay_date<=%ld and pay_date>%ld %s",sCom_branch.br_no,g_pub_tx.tx_date,ldate,cStr);
			/*dAmtA /= 10000;*/
			dAmtA3 += dAmtA;
      ...
      sprintf(str, fmt, t_str);
			break;
		case 'a': /*兑付笔数*/
		  iCountA1  = sql_count("fn_reg","   tx_br_no='%s' and sts ='2' and return_date<=%ld and return_date>%ld %s",sCom_branch.br_no,g_pub_tx.tx_date,ldate,cStr);
			iCountA4 += iCountA1;
      ...
      sprintf(str, fmt, t_str);
			break;
		case 'J': /*兑付金额*/
			ret = sql_sum_double("fn_reg","return_capital",&dAmtA1,"   tx_br_no='%s' and sts ='2' and return_date<=%ld and return_date>%ld %s",sCom_branch.br_no,g_pub_tx.tx_date,ldate,cStr);
			/*ret = sql_sum_double("fn_reg","fct_intst",&dAmtA12,"   tx_br_no='%s' and sts ='2' and return_date<=%ld and return_date>%ld %s",sCom_branch.br_no,g_pub_tx.tx_date,ldate,cStr);
		  ret = sql_sum_double("fn_br_profit","profit_amt",&dAmtA13," pay_date<=%ld and pay_date>%ld and prdt_code in (select prdt_code from fn_parm where  stat='8') %s",g_pub_tx.tx_date,ldate,cStr);
    ...
    case 'C':/*总计本季净募集金额*/
      ...
      break;
    case 'R':/*本季客户端收益*/
    	ret = sql_sum_double("fn_reg","fct_intst",&dAmt3,"tx_br_no = '%s' and return_date<=%ld and return_date>%ld %s",sCom_branch.br_no,g_pub_tx.tx_date,ldate,cStr);
    	/*dAmt3 /= 10000;*/
    	dAmt4 += dAmt3;
     ...
     case 'P':/*总计客户端收益*/
      ...
      break;
    case 'W':/*本季银行端收益*/  
      /*ret = sql_sum_double("fn_br_profit","profit_amt",&dAmt5,"br_no = '%s' and   tx_date<=%ld and tx_date>%ld",sCom_branch.br_no,g_pub_tx.tx_date,ldate);*/
      ret = sql_sum_double("fn_br_profit","profit_amt",&dAmtA13,"br_no='%s' pay_date<=%ld and pay_date>%ld and prdt_code in (select prdt_code from fn_parm where  stat='8') %s",sCom_branch.br_no,g_pub_tx.tx_date,ldate,cStr);      
      dAmt6 += dAmtA13;
