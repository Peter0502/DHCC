...
int get_ratio_dgkhqd(int bli,int bll,char str[100]);
...
rpt937()
{
	...
	pub_base_sysinit();
/*
	strcpy( filename , "BMdgkhqd" );	
	strcpy(vpfm, "dgkhqd" );
*/
	strcpy( filename , "RPT937");
	strcpy(vpfm, "RPT937");
	/* 条件: 交易日期为上一交易日期  的一个月 */
	...
	/**组成查询**/	
	strcat(wherelist," 1=1 ORDER BY opn_br_no,tx_brno,tx_date,prdt_no,wrk_sts,ac_no");
...
OkExit:
	...
	sprintf(acErrMsg,"生成对公开户交易清单成功!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 0;
ErrExit:
	sql_rollback();	/*--事务回滚--*/
	sprintf(acErrMsg,"生成对公开户流水清单失败!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 1;
}
...
{	
	ret=pub_rpt_read_pfm_qd(fp,&line,vpfm,"0001",vrec,&opncnt,get_ratio_dgkhqd);
	ERR_DEAL
...
/* 赋值函数 */
int get_ratio_dgkhqd( bli,bll,str )
int bli,bll;
char str[100];
{ 
	char vhm[101]; 
...
*/			
		...
		case 'K': /* 开户日期 */			
			...
			break;	
		case 'O':  /*  状态  */	
			if(s_mo_opn_cls.wrk_sts[0] =='1')
			{
			  sprintf( t_str , "开户" ); 
		  }
		  else if(s_mo_opn_cls.wrk_sts[0] =='2')
		  {
		  	sprintf( t_str , "销户" ); 
		  }
			sprintf(fmt, "%%-%ds", bll);
