...
rpt975()
{
	...
	/**------- 初始化全局变量 --------**/
	pub_base_sysinit();
	strcpy( filename , "BJdgjxchqd" );	
	strcpy(vpfm, "dgjxchqd" );
	
	ret=Intst_hst_Dec_Sel(g_pub_tx.reply,"ac_ind='0' and intst_date=20030921 and ac_id in (select ac_id from tmp_table where 1=1 )order by br_no");
	ERR_DEAL
	ttlnum=0;
	while(1)
	{
...
OkExit:
	...
	sprintf(acErrMsg,"生成对公计息交易清单成功!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 0;
ErrExit:
	sql_rollback();	/*--事务回滚--*/
	sprintf(acErrMsg,"生成对公计息清单失败!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 1;
}
...
{ 
	...
	{ 
		...
		****/
		...
		case 'K': /* 标志 */
			sprintf(fmt, "%%-%ds", bll);
			if( s_intst_hst.pt_ind[0]=='*' )
				sprintf( str, fmt , "冲回");
			else
				sprintf( str, fmt , "冲回不成功");
			break;
		default : 
			memset( str,' ',bll ); 
			break; 
