...
int get_ratio_lvpkgjj(int bli,int bll,char str[100]);
...
rpt513()
{
  ...
  qry_date = com_sys_parm.lst_date;
	...
	strcpy( filename , "RPT513" );	
	strcpy(vpfm, "rpt513" );

	sprintf( tmpstr," in_date ='%ld' and lw_ind='1' and lv_sts='5'" , qry_date);
	...
	while(1)
	{
		...
		if( ret==100 ) 
		{
			...
			sprintf(acErrMsg,"读取lvpkgreg表异常出错!");
			WRITEMSG			
			goto ErrExit;			
		}
		...
		if( strcmp( lv_pkgreg.br_no , old_br_no ) )
		{
    		...
    		/* 统计信息 */
    cnt = sql_count("lv_pkgreg"," in_date='%ld' and br_no ='%s' and lw_ind='1' and lv_sts='5' ",qry_date,old_br_no);
...
OkExit:
	...
	sprintf(acErrMsg,"查询小额支付类往帐拒绝清单成功!!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 0;
ErrExit:
	sql_rollback();	/*--事务回滚--*/
	sprintf(acErrMsg,"查询小额支付类往帐拒绝清单失败!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 1;
}
...
{	
	...
	ret = pub_rpt_read_pfm_qd(fp,&line,vpfm,"0001",vrec,&opncnt,get_ratio_lvpkgjj);
	ERR_DEAL
...
/* 赋值函数 */
int get_ratio_lvpkgjj( bli,bll,str )
int bli,bll;
char str[100];
{
	char vstr[101]; 
  ...
  memset(cTx_Desc,0x00,	sizeof(cTx_Desc));
	...
	{ 
		case 'A': /* 交易日期 */
			...
			vtcp_log("[%s][%d]get_ratio_lvpkgjj begin in_date=[%s]",__FILE__,__LINE__,str);
			break;
		case 'B': /* 机构编码 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str , fmt , lv_pkgreg.br_no ); 
			vtcp_log("[%s][%d]get_ratio_lvpkgjj begin br_no=[%s]",__FILE__,__LINE__,str);
			break; 
		case 'C': /* 机构名称 */
		  ...
		  lv_pkgreg.br_no);
			...
			sprintf(fmt, "%%-%ds", bll);
			sprintf(str, fmt, t_str);
			vtcp_log("[%s][%d]get_ratio_lvpkgjj begin br_name=[%s]",__FILE__,__LINE__,str);
			break;
		case 'D': /* 页码 */
			sprintf(fmt,"%%%dld",bll); 
			sprintf(str, fmt, page); 
			vtcp_log("[%s][%d]get_ratio_lvpkgjj begin page=[%d]",__FILE__,__LINE__,str);
			break; 
		case 'E': /* PKGNO 
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_pkgreg.pkgno);
			vtcp_log("[%s][%d]get_ratio_lvpkgjj begin pkgno=[%s]",__FILE__,__LINE__,str);
			break;
			**/
		...
		case 'F': /* 交易序号 */						
			...
			vtcp_log("[%s][%d]get_ratio_lvpkgjj begin orderno=[%s]",__FILE__,__LINE__,str);
			break;
		case 'G': /* 发起行行号 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_pkgreg.or_br_no);
			vtcp_log("[%s][%d]get_ratio_lvpkgjj begin or_br_no=[%s]",__FILE__,__LINE__,str);
			break;
		case 'H': /* 付款人帐号 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_pkgreg.pay_ac_no);
			vtcp_log("[%s][%d]get_ratio_lvpkgjj begin pay_ac_no=[%s]",__FILE__,__LINE__,str);
			break;
		case 'I': /* 付款人名称 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_pkgreg.pay_name);
			vtcp_log("[%s][%d]get_ratio_lvpkgjj begin pay_name=[%s]",__FILE__,__LINE__,str);
			break;
		case 'J': /* 接收行行号 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_pkgreg.ac_br_no); 
			vtcp_log("[%s][%d]get_ratio_lvpkgjj begin ac_br_no=[%s]",__FILE__,__LINE__,str);
			break;
		case 'K': /* 交易金额 */
			/* 加逗号 */
			...
			vtcp_log("[%s][%d]get_ratio_lvpkgjj begin tx_amt=[%.2lf]",__FILE__,__LINE__,str);
			break;	
		case 'L': /* 收款人帐号 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_pkgreg.cash_ac_no);
			vtcp_log("[%s][%d]get_ratio_lvpkgjj begin cash_ac_no=[%s]",__FILE__,__LINE__,str);
			break;
		case 'M': /* 收款人名称 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_pkgreg.cash_name);
			vtcp_log("[%s][%d]get_ratio_lvpkgjj begin cash_name=[%s]",__FILE__,__LINE__,str);
			break;
		case 'N': /* 包序号 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_pkgreg.packid);
			vtcp_log("[%s][%d]get_ratio_lvpkgjj begin packid=[%s]",__FILE__,__LINE__,str);
			break;
		case 'O': /* 包日期 */
			sprintf(fmt,"%%-%dld",bll); 
