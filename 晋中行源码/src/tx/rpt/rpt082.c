...
int get_ratio_lnjhkkhd(int bli,int bll,char str[100]);
...
rpt082()
{
	...
	strcpy( filename , "BRlnjhkkhd" );	
	strcpy(vpfm, "lnjhkkhd" );
	
	/* 本金积数结息清单 */
	/* 条件:tx_date为当日,tx_code='G082',增减标志为减少 */
	memset( &tmpstr,0x0,strlen(tmpstr));	
	memset( &wherelist,0x0,strlen(wherelist));
	sprintf( tmpstr," tx_code='G082' and tx_date=%ld and add_ind='0' and ln_tx_type='0' and " ,qry_date );
...
OkExit:
	...
	sprintf(acErrMsg,"生成日终贷款客户回单(按还款计划扣划)成功!!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 0;
ErrExit:
	sql_rollback();	/*--事务回滚--*/
	sprintf(acErrMsg,"生成日终贷款客户回单(按还款计划扣划)失败!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 1;
}
...
{	
	ret = pub_rpt_read_pfm_qd(fp,&line,vpfm,"0001",vrec,&opncnt,get_ratio_lnjhkkhd);
	ERR_DEAL
...
/* 赋值函数 */
int get_ratio_lnjhkkhd( bli,bll,str )
int bli,bll;
char str[100];
{ 
	char vhm[101]; 
	...
	{ 
		...
		case 'J': /* 还本金额(大写) */
			sprintf(fmt,"%%-%ds",bll);
			sprintf(str, fmt, pub_rpt_pcask(bal_amt));
			break;
		case 'K': /* 还本金额(小写) */
			/* 加逗号 */
