...
int get_ratio_lvpkgwz(int bli,int bll,char str[100]);
...
rpt510()
{
  ...
  qry_date = com_sys_parm.lst_date;
	...
	strcpy( filename , "RPT510" );	
	strcpy(vpfm, "rpt510" );

	sprintf( tmpstr," in_date ='%ld' and lw_ind='1'" , qry_date);
	...
	while(1)
	{
	  ...
	  vtcp_log("[%s][%d] here line_file ",__FILE__,__LINE__,line_file);
		...
		if( strcmp( lv_pkgreg.br_no , old_br_no ) )
		{
    		...
    		/* 统计信息 */
    cnt = sql_count("lv_pkgreg"," in_date='%ld' and br_no ='%s' and lw_ind='1' ",qry_date,old_br_no);
    vtcp_log("[%s][%d]机构号为[%s]的总共[%d]笔当前打印第[%d]笔",__FILE__,__LINE__,old_br_no,cnt,line_file);
		...
		line_file++;
		/* if( line_file>8 )	 大于8行换页del by martin 070519 */
		if( line_file>6 )	/* 大于6行换页 */
		{
			line_file=0;
			page++;
			/* 打印页尾 */
...
OkExit:
	...
	sprintf(acErrMsg,"查询小额lv_pkgreg往帐明细成功!!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 0;
ErrExit:
	sql_rollback();	/*--事务回滚--*/
	sprintf(acErrMsg,"查询小额支付交易往帐明细失败!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 1;
}
...
{	
	...
	ret = pub_rpt_read_pfm_qd(fp,&line,vpfm,"0001",vrec,&opncnt,get_ratio_lvpkgwz);
	ERR_DEAL
...
/* 赋值函数 */
int get_ratio_lvpkgwz( bli,bll,str )
int bli,bll;
char str[100];
{
	char vstr[101]; 
  ...
  memset(cTx_Desc    ,0x00,	sizeof(cTx_Desc));
	...
	{ 
		case 'A': /* 交易日期 */
			...
			vtcp_log("[%s][%d]get_ratio_lvpkgwz begin in_date=[%s]",__FILE__,__LINE__,str);
			break;
		case 'B': /* 机构编码 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str , fmt , lv_pkgreg.br_no ); 
			vtcp_log("[%s][%d]get_ratio_lvpkgwz begin br_no=[%s]",__FILE__,__LINE__,str);
			break; 
		case 'C': /* 机构名称 */
		  ...
		  lv_pkgreg.br_no);
			...
			sprintf(fmt, "%%-%ds", bll);
			sprintf(str, fmt, t_str);
			vtcp_log("[%s][%d]get_ratio_lvpkgwz begin br_name=[%s]",__FILE__,__LINE__,str);
			break;
		case 'D': /* 页码 */
			sprintf(fmt,"%%%dld",bll); 
			sprintf(str, fmt, page); 
			vtcp_log("[%s][%d]get_ratio_lvpkgwz begin page=[%d]",__FILE__,__LINE__,str);
			break; 
		case 'E': /* PKGNO */
			/**
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_pkgreg.pkgno);
			vtcp_log("[%s][%d]get_ratio_lvpkgwz begin pkgno=[%s]",__FILE__,__LINE__,str);
			break;
			***/
		...
		case 'F': /* 交易序号 */						
			...
			vtcp_log("[%s][%d]get_ratio_lvpkgwz begin orderno=[%s]",__FILE__,__LINE__,str);
			break;
		case 'G': /* 发起行行号 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_pkgreg.or_br_no);
			vtcp_log("[%s][%d]get_ratio_lvpkgwz begin or_br_no=[%s]",__FILE__,__LINE__,str);
			break;
		case 'H': /* 付款人帐号 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_pkgreg.pay_ac_no);
			vtcp_log("[%s][%d]get_ratio_lvpkgwz begin pay_ac_no=[%s]",__FILE__,__LINE__,str);
			break;
		case 'I': /* 付款人名称 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_pkgreg.pay_name);
			vtcp_log("[%s][%d]get_ratio_lvpkgwz begin pay_name=[%s]",__FILE__,__LINE__,str);
			break;
		case 'J': /* 接收行行号 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_pkgreg.ac_br_no); 
			vtcp_log("[%s][%d]get_ratio_lvpkgwz begin ac_br_no=[%s]",__FILE__,__LINE__,str);
			break;
		case 'K': /* 交易金额 */
			/* 加逗号 */
			...
			vtcp_log("[%s][%d]get_ratio_lvpkgwz begin tx_amt=[%.2lf]",__FILE__,__LINE__,str);
			break;	
		case 'L': /* 收款人帐号 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_pkgreg.cash_ac_no);
			vtcp_log("[%s][%d]get_ratio_lvpkgwz begin cash_ac_no=[%s]",__FILE__,__LINE__,str);
			break;
		case 'M': /* 收款人名称 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_pkgreg.cash_name);
			vtcp_log("[%s][%d]get_ratio_lvpkgwz begin cash_name=[%s]",__FILE__,__LINE__,str);
			break;
		case 'N': /* 包序号 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_pkgreg.packid);
			vtcp_log("[%s][%d]get_ratio_lvpkgwz begin packid=[%s]",__FILE__,__LINE__,str);
			break;
		case 'O': /* 包日期 */
			sprintf(fmt,"%%-%dld",bll); 
			sprintf( str, fmt, lv_pkgreg.pack_date);
			vtcp_log("[%s][%d]get_ratio_lvpkgwz begin pack_date=[%ld]",__FILE__,__LINE__,str);
			break;	
		case 'P': /* 记录状态 
			sprintf(fmt,"%%-%dld",bll); 
