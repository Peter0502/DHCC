...
rpt939()
{
	...
	memset( &gTd_mst,0x00,sizeof(struct td_mst_c));	
	...
	strcpy( filename , "RPT939");
	strcpy(vpfm, "RPT939");
	
	/**组成查询**/
	memset(wherelist,'\0',sizeof(wherelist));
	strcat(wherelist," bal != 0.0 and ac_id in (select ac_id from mdm_ac_rel where \
                     substr(ac_no,1,1)= '5') order by opn_br_no,prdt_no,bal ") ;
	...
	while(1)
	{
		ret=Td_mst_Fet_Sel( &gTd_mst, g_pub_tx.reply );
	...
	vtcp_log("打印页尾1:[%s]--[%d]",__FILE__,__LINE__);
		...
		/* 机构变换时 */
		pub_base_strpack( gTd_mst.opn_br_no );
		if( strcmp( gTd_mst.opn_br_no , old_br_no ) )
		{
			/* 第一个机构 */
			if( strcmp( old_br_no , "*****" ) )
			{
			...
			strcpy( old_br_no , gTd_mst.opn_br_no );
			
			page=1;
			line_file=1;
			
			/* 删除原文件 */
			ret = pub_rpt_rmfile( gTd_mst.opn_br_no, filename , 0 );
			ERR_DEAL
			/* 打开文件 */	
			ret = pub_rpt_openfile( &fp , gTd_mst.opn_br_no, filename );
			ERR_DEAL
			/* 打印页首 */
			ret = print_head();
			ERR_DEAL
...
char str[200];
{
	...
	{ 
		case 'A': /* 交易日期 */
			...
			break;
		case 'C': /* 机构名称 */
			ret = Com_branch_Sel(g_pub_tx.reply, &sCom_branch, "br_no='%s'",gTd_mst.opn_br_no);
			if (ret != 0)
			{
			...
			sprintf( t_str , "%s    ( %5s )" , sCom_branch.br_name ,gTd_mst.opn_br_no); 
			sprintf(fmt, "%%-%ds", bll);
			sprintf(str, fmt, t_str);
			break;
		case 'D': /* 页码 */
			...
			break;
		case 'M': /*新帐号*/
			ret1=Mo_merge_ac_Sel(g_pub_tx.reply,&sMo_merge_ac,"ac_id=%d",gTd_mst.ac_id);
			if(ret1 == 100)
			{
				ret2=Mdm_ac_rel_Sel(g_pub_tx.reply,&sMdm_ac_rel,"ac_id=%d",gTd_mst.ac_id);
				if(ret2)
				{
		...
		case 'I': /*旧帐号*/
			...
			break;
		case 'N': /*余额*/
			sprintf(t_str,"%.2lf",gTd_mst.bal);
			pub_rpt_flttomon(t_str,str_mon);
			sprintf(fmt,"%%-%ds",bll);
			sprintf(str,fmt,str_mon);
			break;	
		case 'L':  /*户      名 */
			pub_base_strpack(gTd_mst.name); 
			sprintf( t_str , "%s" , gTd_mst.name); 
			sprintf(fmt, "%%-%ds", bll);
			sprintf( str, fmt, t_str);
			break;
		case 'O': /*每页小计*/
