	...
	if(!memcmp(s_gl_sub.br_no,"59801",5))
	{
		...
		if( s_gl_sub.acc_hrt[0] != old_acc_head[0] )
		{
			if( old_acc_head[0] == 'A' )				/* 资产类 */
			{
				...
				/*** add by martin 科目無餘額***/ /*del by lwb 20150202*/
/*ret=Gl_sub_Sel( g_pub_tx.reply , &sGl_sub , "br_no='%s' and cur_no='01' and acc_hrt like '%s%%' \
...
/*		case 'Y':  汇总 
		...
		case 'K': /* 月初借余额 */
			...
			if( flag==0 )
			{
				ret = sql_sum_double( "gl_sub","mdr_bal",&tmp_amt,"acc_hrt[1]='%c' and br_no='%s' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) ", acc_type[0],s_gl_sub.br_no );
			 /* if( acc_type[0]=='4' )
			  {
			...
			{
				ret = sql_sum_double( "gl_sub","mdr_bal",&tmp_amt,"acc_hrt[1]>='1' and br_no='%s' and acc_hrt[1]<='5' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) " ,s_gl_sub.br_no);
			  /*ret = sql_sum_double( "gl_sub","mdr_bal",&tmp_amt1,"acc_hrt='40602' and br_no='%s' ",s_gl_sub.br_no);
				tmp_amt+=tmp_amt1;*/
			}
			ERR_DEAL
		...
		case 'L': /* 月初贷余额 */
			if( flag==0 )
			{
			  ...
			  ret = sql_sum_double( "gl_sub","mcr_bal",&tmp_amt,"substr(acc_hrt,1,1)='%c' and br_no='%s' and acc_hrt in ( select acc_hrt from com_item where acc_lvl='1' )  ", acc_type[0] ,s_gl_sub.br_no);
			  /*if(acc_type[0]=='4')
			  {
			...
			else 
			{
			  ...
			  ret = sql_sum_double( "gl_sub","mcr_bal",&tmp_amt,"substr(acc_hrt,1,1)>='1' and br_no='%s' and substr(acc_hrt,1,1)<='5' and acc_hrt in ( select acc_hrt from com_item where acc_lvl='1' ) ",s_gl_sub.br_no );
			  /*ret = sql_sum_double( "gl_sub","mcr_bal",&tmp_amt1,"acc_hrt='40601' and br_no='%s' ",s_gl_sub.br_no);
				tmp_amt+=tmp_amt1;*/
			}
			ERR_DEAL
		...
		case 'O': /* 本月借发生额 */
			if( flag==0  )
			{
				ret = sql_sum_double( "gl_sub","mdr_amt",&tmp_amt,"acc_hrt[1]='%c' and br_no='%s' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) ", acc_type[0] ,s_gl_sub.br_no);
			}
			else
			{
				ret = sql_sum_double( "gl_sub","mdr_amt",&tmp_amt,"acc_hrt[1]>='1' and br_no='%s' and acc_hrt[1]<='5' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) ",s_gl_sub.br_no );
			}
			ERR_DEAL
		...
		case 'P': /* 本月贷发生额 */
			if( flag==0 )
			{
				ret = sql_sum_double( "gl_sub","mcr_amt",&tmp_amt,"acc_hrt[1]='%c' and br_no='%s' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' )", acc_type[0],s_gl_sub.br_no );
			}
			else
			{
				ret = sql_sum_double( "gl_sub","mcr_amt",&tmp_amt,"acc_hrt[1]>='1' and br_no='%s' and acc_hrt[1]<='5' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) " ,s_gl_sub.br_no);
			}
			ERR_DEAL
		...
		case 'V': /* 月末贷余额 */
			...
			break;
	 case 'Y': /* 月初余额 */
			sprintf( amt_tmp, "%.2lf" , s_gl_sub.mdr_bal+s_gl_sub.mcr_bal ); 
			if( !pub_base_CompDblVal(atof(amt_tmp),0.00) )
			{
				memset(str,' ',bll );
				break;
