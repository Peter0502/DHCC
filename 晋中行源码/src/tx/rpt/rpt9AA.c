...
rpt9AA()
{
  ...
  char filename1[64];    
    ...
    ERR_DEAL
    if(beg_tx_date == 0) /****如果上面操作中取不到日期，日期就设为融信试营业时的大概日期  20110104****/
    {
	beg_tx_date = 20100601;
    }
         
    /****先写死了***/
/*  beg_tx_date=20070731; */
    ...
    while(1)
    {
        ...
        /*if(memcmp(pre_acc_hrt,s_dc_log_bk.acc_hrt,5) ||memcmp(pre_br_no,s_dc_log_bk.tx_tx_br_no,5) )*/
        if(memcmp(pre_acc_hrt,s_dc_log_bk.acc_hrt,5) ||memcmp(pre_br_no,s_dc_log_bk.tx_opn_br_no,5) )
        {
            /**初始化结构体 2007-5-4 11:08**/
            memset(&s_gl_mst_hst,0x00,sizeof(s_gl_mst_hst)); 
            ret = Gl_mst_hst_Sel(g_pub_tx.reply,&s_gl_mst_hst,"acc_hrt='%s' and br_no='%s' and date=%ld", s_dc_log_bk.acc_hrt,s_dc_log_bk.tx_opn_br_no,s_dc_log_bk.tx_date);
            if(ret && ret != 100)
            {
            ...
            d_bal = 0.0;
           vtcp_log("[%s][%d]   bal=[%.2lf]\n,s_gl_mst_hst.ldd_bal=[%.2lf]\n,s_gl_mst_hst.lcd_bal=[%.2lf]\n",__FILE__,__LINE__,d_bal, s_gl_mst_hst.ldd_bal,s_gl_mst_hst.lcd_bal);
            
            if(s_com_item.dc_ind[0]=='0' || s_com_item.dc_ind[0]=='1')
                d_bal = s_gl_mst_hst.ldd_bal-s_gl_mst_hst.lcd_bal;
            else if(s_com_item.dc_ind[0]=='2')
                d_bal = s_gl_mst_hst.lcd_bal- s_gl_mst_hst.ldd_bal;
           vtcp_log("[%s][%d]   bal=[%.2lf],log_bal=[%.2lf]\n",__FILE__,__LINE__,d_bal,s_acc_hrt_log.bal);
        ...
        memcpy(s_acc_hrt_log.ct_ind       ,s_dc_log_bk.ct_ind       ,sizeof(s_acc_hrt_log.ct_ind )-1);    
        ...
        s_acc_hrt_log.pagecnt        =        0;
            pre_trace_no=s_dc_log_bk.trace_no;
            pre_trace_cnt=s_dc_log_bk.trace_cnt;
    
     memset(&s_trace_log_bk,0x00,sizeof(s_trace_log_bk));
     /***************ligl2008-2-27 17:41*********************
     if(memcmp(s_dc_log_bk.entry_code,"DEWZ00",5)==0 || memcmp(s_dc_log_bk.entry_code,"DELZ00",5)==0 || memcmp(s_dc_log_bk.entry_code,"XEWZ00",5) ==0|| memcmp(s_dc_log_bk.entry_code,"XEJZ00",5)==0)
     {
            memcpy( s_acc_hrt_log.brf         ,"         "           ,sizeof(s_trace_log_bk.brf)-1);
     }
     else
     {
         ***************end************************
     ret=Trace_log_rz_Dec_Sel(g_pub_tx.reply,"tx_date=%ld and trace_no=%ld and trace_cnt=%ld ", s_dc_log_bk.tx_date,s_dc_log_bk.trace_no,s_dc_log_bk.trace_cnt);
         while(1)
         {
           ret=Trace_log_rz_Fet_Sel(&s_trace_log_bk,g_pub_tx.reply);
           if(ret && ret!=100)
          {
              vtcp_log("[%s][%d]数据库操作错误!\n",__FILE__,__LINE__);
              strcpy(g_pub_tx.reply,"P001");
          ...
          if(ret==100)
           ...
           memcpy( s_acc_hrt_log.brf         ,s_trace_log_bk.brf           ,sizeof(s_trace_log_bk.brf)-1); 
            break;
     }
     Trace_log_rz_Clo_Sel();
     **********************ligl 2008-2-27 17:41**********************
    }
    **********************end***********************/
    /*****新64位上结构体，生产数据库中不存在brf字段，注释掉
                memcpy( s_acc_hrt_log.brf         ,s_dc_log_bk.brf          ,sizeof(s_trace_log_bk.brf)-1);
		*****/
        ...
        vtcp_log("bal=[%.2lf],log_bal=[%.2lf]\n",d_bal,s_acc_hrt_log.bal);
      /*printf(" s_acc_hrt_log.tx_date[%d],s_acc_hrt_log.acc_hrt[%s],s_acc_hrt_log.brf[%s]\n"    ,s_acc_hrt_log.tx_date,s_acc_hrt_log.acc_hrt, s_acc_hrt_log.brf);*/
       /* get_zd_long("1261",&s_acc_hrt_log.tt_no);     **add by zhanghongbo 20090819***/
