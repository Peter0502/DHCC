...
int get_ratio_lnajxxqd(int bli,int bll,char str[100]);
...
rpt031()
{
	...
	memset( &s_ln_pay_pln_tmp , 0x00 , sizeof(struct ln_pay_pln_tmp_c));
	...
	strcpy( filename , "RPT031" );	
	strcpy(vpfm, "RPT031" );
		
	ret= Ln_pay_pln_tmp_Sel( g_pub_tx.reply,&s_ln_pay_pln_tmp," pay_ind='0' ");
	if( ret !=0 && ret!=100)
	{
		sprintf(acErrMsg,"读取按揭欠款表信息异常出错! ret=%d g_pub_tx.reply=%s",ret,g_pub_tx.reply);
		WRITEMSG			
		goto ErrExit;
	}else if(ret==100){/*JYW 如果没有则不出表直接退出*/
		goto OkExit; 
		}
		else{
		wrk_date=s_ln_pay_pln_tmp.wrk_date;
		end_date=s_ln_pay_pln_tmp.end_date;
	}
	memset( &s_ln_pay_pln_tmp , 0x00 , sizeof(struct ln_pay_pln_tmp_c));
			
	/**组成查询**/	
	/**根据王师傅要求，销户的不再打印begin 
	strcpy( wherelist," repay_type in ('3','4') ORDER BY opn_br_no,prdt_no,opn_date,ac_id,ac_seqn");  
	end  by jk 20111106**/
	strcpy( wherelist," repay_type in ('3','4') and ac_sts<>'*' and ac_sts<>'9' ORDER BY opn_br_no,prdt_no,opn_date,ac_id,ac_seqn");
	...
	while(1)
	{
		...
		if( strcmp( s_ln_mst.opn_br_no , old_br_no ) )
		{
    		...
    		ret= Ln_pay_pln_tmp_Sel( g_pub_tx.reply,&s_ln_pay_pln_tmp,"ac_id=%ld and ac_seqn=%d and pay_ind='0' ",s_ln_mst.ac_id,s_ln_mst.ac_seqn);
		if( ret&&ret!=100 )
		{
		...
		}else if( ret==100 )
		{
			...
			s_ttl_cnt=s_ln_pay_pln_tmp.ttl_cnt;
			s_now_cnt=s_ln_pay_pln_tmp.curr_cnt;
		}
...
{	
	ret=pub_rpt_read_pfm_qd(fp,&line,vpfm,"0001",vrec,&opncnt,get_ratio_lnajxxqd);
	ERR_DEAL
...
/* 赋值函数 */
int get_ratio_lnajxxqd( bli,bll,str )
int bli,bll;
char str[100];
{ 
	char vhm[101]; 
	...
	{ 
		case 'A': /* 交易日期 */
			sprintf( t_str , "%4d年%2d月%2d日 -- %4d年%2d月%2d日" , wrk_date/10000 , wrk_date%10000/100 , wrk_date%100 , end_date/10000 , end_date%10000/100 , end_date%100);
			sprintf(fmt,"%%-%ds",bll); 
		...
		case 'K': /* 本期利息 */			
			...
			break;	
		case 'P': /* 期数 */
			sprintf( t_str , "%3d/%3d" ,s_now_cnt,s_ttl_cnt);
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str , fmt , t_str ); 
			break;
		case 'Q': /* 总贷款金额 */
