...
#include "dd_parm_c.h"                  /*增加科目号、积数，zhangpeng 20131125*/
#include "dc_acc_rel_c.h"								/*增加科目号、积数，zhangpeng 20131126*/
#include "com_item_c.h"									/*增加科目号、积数，zhangpeng 20131126*/
...
rpt929()
{
	...
	/**组成查询**/
	memset(wherelist,'\0',sizeof(wherelist));
	strcat(wherelist," dd_mst.bal != 0.0 and  dd_mst.ac_type<>'5' and dd_mst.ac_id=mdm_ac_rel.ac_id  order by dd_mst.opn_br_no,mdm_ac_rel.ac_no ") ;

	vtcp_log( "WHERE [%s]",wherelist );
	
	ret= Mdm_ac_rel_dd_mst_Dec_Sel( g_pub_tx.reply, wherelist );
	ERR_DEAL
	...
	while(1)
	{
		ret=Mdm_ac_rel_dd_mst_Fet_Sel( &gDd_mst,&sMdm_ac_rel ,g_pub_tx.reply );
...
char str[200];
{
	...
	struct dd_parm_c s_dd_parm;							/*增加科目号、积数，zhangpeng 20131126*/
	struct dc_acc_rel_c s_dc_acc_rel;				/*增加科目号、积数，zhangpeng 20131126*/
	struct com_item_c s_com_item;						/*增加科目号、积数，zhangpeng 20131126*/
	int ret1=0,ret2=0;
	char str_mon[30];
	...
	memset(&s_dd_parm, 0x0, sizeof(struct dd_parm_c));			/*增加科目号、积数，zhangpeng 20131126*/
	memset(&s_dc_acc_rel, 0x0, sizeof(struct dc_acc_rel_c));	/*增加科目号、积数，zhangpeng 20131126*/
	memset(&s_com_item, 0x0, sizeof(struct com_item_c));			/*增加科目号、积数，zhangpeng 20131126*/
	...
	{ 
		...
		case 'O': /*每页小计*/
			...
			sprintf(str,fmt,str_mon);
			break;
		/**增加科目号、积数。zhangpeng 20131126 start***/
		case 'P': /*科目号*/
			ret=Dd_parm_Sel( g_pub_tx.reply ,&s_dd_parm,"prdt_no='%s'",gDd_mst.prdt_no);
			if( ret )
			{
				sprintf(acErrMsg,"查询活期存款主文件表出错!![%d]",ret);
				WRITEMSG
				goto ErrExit;
			}
			/* 加and substr(acc_hrt,1,3)<>'521'  and substr(acc_hrt,1,3)<>'522' 取科目时 不要取到利息科目 wudawei 20141114*/
			ret=Dc_acc_rel_Sel( g_pub_tx.reply ,&s_dc_acc_rel,"dc_code='%s' and substr(acc_hrt,1,3)<>'521'  and substr(acc_hrt,1,3)<>'522' ",s_dd_parm.dc_code);
			if( ret )
			{
			...
			memset( &s_dd_parm, 0x00 , sizeof( struct ln_parm_c ));
			memset( &s_dc_acc_rel, 0x00 , sizeof( struct dc_acc_rel_c ));
			memset( &s_com_item, 0x00 , sizeof( struct com_item_c ));
			break;
		case 'Q': /*利息积数*/
			sprintf(t_str,"%17.0lf",gDd_mst.intst_acm);
			sprintf(fmt,"%%-%ds",bll);
			sprintf(str,fmt,t_str);
			break;
		/**增加科目号、积数。zhangpeng 20131126 end***/
		default : 
			memset( str,' ',bll ); 
			break; 
	}
