...
int get_ratio_lnqkkkqd(int bli,int bll,char str[100]);
...
rpt044()
{
	...
	pub_base_sysinit();
/*	
	strcpy( filename , "BRlnqkkkqd" );	
	strcpy(vpfm, "lnqkkkqd" );
*/	
	strcpy( filename , "RPT044");
	strcpy(vpfm, "RPT044");
	/* 本金积数结息清单 */
	/* 条件:tx_date为当日,tx_code='G086',增减标志为减少 */
	tol_jx_cnt=0;
	tol_bal_amt=0.00;
	tol_in_amt=0.00;	
	tol_out_amt=0.00;	
	...
	sprintf( tmpstr," tx_code = 'G086' and tx_date=%ld and add_ind='0' and " ,qry_date );
...
OkExit:
	...
	sprintf(acErrMsg,"生成按揭欠款扣款清单成功!!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 0;
ErrExit:
	sql_rollback();	/*--事务回滚--*/
	sprintf(acErrMsg,"生成按揭欠款扣款清单失败!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 1;
}
...
{	
	ret = pub_rpt_read_pfm_qd(fp,&line,vpfm,"0001",vrec,&opncnt,get_ratio_lnqkkkqd);
	ERR_DEAL
...
/* 赋值函数 */
int get_ratio_lnqkkkqd( bli,bll,str )
int bli,bll;
char str[100];
{ 
	char vhm[101]; 
