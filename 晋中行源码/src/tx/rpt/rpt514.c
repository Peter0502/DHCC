...
int get_ratio_lvwbctl(int bli,int bll,char str[100]);
...
rpt514()
{
  ...
  qry_date = com_sys_parm.lst_date;
	...
	strcpy( filename , "RPT514" );	
	strcpy(vpfm, "rpt514" );

	sprintf( tmpstr," in_date ='%ld'  " , qry_date);
	strcat( wherelist,tmpstr );	

	strcat(wherelist," order by br_no,stat,packid ");
	...
	while(1)
	{
		...
		if( ret==100 ) 
		{
			...
			sprintf(acErrMsg,"读取lv_wbctl表异常出错!");
			WRITEMSG			
			goto ErrExit;			
		}
    vtcp_log("[%s][%d]lv_wbctl.brno=[%s]",__FILE__,__LINE__,lv_wbctl.br_no);
		/* 机构变换时 */
    ...
    /* 统计信息 */
    vtcp_log("[%s][%d]lv_wbctl.brno=[%s]",__FILE__,__LINE__,lv_wbctl.br_no);
    cnt = sql_count("lv_wbctl"," in_date ='%ld' and br_no='%s' ",qry_date,old_br_no);
...
OkExit:
	...
	sprintf(acErrMsg,"查询小额支付交易往包成功!!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 0;
ErrExit:
	sql_rollback();	/*--事务回滚--*/
	sprintf(acErrMsg,"查询小额支付交易往包失败!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 1;
}
...
{	
	...
	ret = pub_rpt_read_pfm_qd(fp,&line,vpfm,"0001",vrec,&opncnt,get_ratio_lvwbctl);
	ERR_DEAL
...
/* 赋值函数 */
int get_ratio_lvwbctl( bli,bll,str )
int bli,bll;
char str[100];
{
	char vstr[101]; 
	...
	{ 
		case 'A': /* 交易日期 */
			...
			vtcp_log("[%s][%d]get_ratio_lvpkgwb begin in_date=[%s]",__FILE__,__LINE__,str);
			break;
		case 'B': /* 机构编码 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str , fmt , lv_wbctl.br_no ); 
			vtcp_log("[%s][%d]get_ratio_lvpkgwb begin br_no=[%s]",__FILE__,__LINE__,str);
			break; 
		case 'C': /* 机构名称 */
		  ...
		  lv_wbctl.br_no);
			...
			sprintf(fmt, "%%-%ds", bll);
			sprintf(str, fmt, t_str);
			vtcp_log("[%s][%d]get_ratio_lvpkgwb begin br_name=[%s]",__FILE__,__LINE__,str);
			break;
		case 'D': /* 页码 */
			sprintf(fmt,"%%%dld",bll); 
			sprintf(str, fmt, page); 
			vtcp_log("[%s][%d]get_ratio_lvpkgwb begin page=[%d]",__FILE__,__LINE__,str);
			break; 
		case 'E': /* PKGNO 
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_wbctl.pkgno);
			vtcp_log("[%s][%d]get_ratio_lvpkgwb begin pkgno=[%s]",__FILE__,__LINE__,str);
			break;
			**/
		...
		case 'F': /* 包序号 */						
			...
			vtcp_log("[%s][%d]get_ratio_lvpkgwb begin packid=[%s]",__FILE__,__LINE__,str);
			break;
		case 'G': /* 包日期 */
			sprintf(fmt,"%%-%dld",bll); 
			sprintf( str, fmt, lv_wbctl.pack_date);
			vtcp_log("[%s][%d]get_ratio_lvpkgwb begin pack_date=[%ld]",__FILE__,__LINE__,str);
			break;	
		case 'H': /* 付款清算行 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_wbctl.pay_qs_no);
			vtcp_log("[%s][%d]get_ratio_lvpkgwb begin pay_qs_no=[%s]",__FILE__,__LINE__,str);
			break;
		case 'I': /* 接收清算行 */
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_wbctl.cash_qs_no);
			vtcp_log("[%s][%d]get_ratio_lvpkgwb begin cash_qs_no=[%s]",__FILE__,__LINE__,str);
			break;
		case 'J': /* 总金额 */
			/* 加逗号 */
			...
			vtcp_log("[%s][%d]get_ratio_lvpkgwb begin totamt=[%.2lf]",__FILE__,__LINE__,str);
			break;
		case 'K': /* 总笔数 */
			sprintf(fmt,"%%-%dld",bll); 
			sprintf( str, fmt, lv_wbctl.dtlcnt);
			vtcp_log("[%s][%d]get_ratio_lvpkgwb begin dtlcnt=[%ld]",__FILE__,__LINE__,str);
			break;		
		case 'L': /* 成功总金额 */
			/* 加逗号 */
			...
			sprintf(fmt,"%%%ds",bll); 		
			sprintf( str, fmt, amt_tmp ); 
			vtcp_log("[%s][%d]get_ratio_lvpkgwb begin dtlsucamt=[%.2lf]",__FILE__,__LINE__,str);
			break;
		case 'M': /* 成功总笔数 */
			sprintf(fmt,"%%-%dld",bll); 
			sprintf( str, fmt, lv_wbctl.dtlsuccnt);
			vtcp_log("[%s][%d]get_ratio_lvpkgwb begin dtlsuccnt=[%ld]",__FILE__,__LINE__,str);
			break;	
		case 'N': /* 包处理状态 
			sprintf(fmt,"%%-%ds",bll); 
			sprintf( str, fmt, lv_wbctl.packstat);
			vtcp_log("[%s][%d]get_ratio_lvpkgwb begin packstat=[%s]",__FILE__,__LINE__,str);
			break;
			**/
			vtcp_log("[%s][%d]开始打印--交易状态!\n",__FILE__,__LINE__);
			memset(cTx_Desc,0x00,sizeof(cTx_Desc));
			vtcp_log("[%s][%d]cTx_Desc==[%s]",__FILE__,__LINE__,cTx_Desc);
