...
rptHP1()
{
	...
	g_reply_int = Hv_poinfo_Dec_Sel(g_pub_tx.reply, \
		"l_tx_date=%ld  and po_sts='1'  ORDER BY br_no  ",sComSysParm.lst_date );
	if( g_reply_int ) 
	{ 
	...
	{ 
		...
		sum_amt+=hv_poinfo.po_bal;/**add by wangyongwei*/
...
char mygsm[3];
{
	...
	switch( bli ) 
	{
		...
		case 'T':
			...
			vtcp_log("[%s][%d]mygsm===[%s],bli=[%c],bll=[%d]\n",__FILE__,__LINE__,mygsm,(char)bli,bll);
			memcpy(vstr,hv_poinfo.pay_name,sizeof(hv_poinfo.pay_name)-1);
			vtcp_log("[%d][%d][%d][%d]\n",bll,bll,bll,bll);
			vtcp_log("[%s][%d]name===[%s]\n",__FILE__,__LINE__,hv_poinfo.pay_name);
			vtcp_log("[%s][%d]vstr===[%s] bll =[%d]\n",__FILE__,__LINE__,vstr,bll);
			deal_half_charcter(vstr);
			cDeal_str(vstr, (sizeof(hv_poinfo.pay_name)-1));
			memcpy(str,vstr,bll);
    ...
    E-挂失 F-解挂失 G-逾期未用退回 H-未用解 I-未用冲帐冲帐             	*/
		...
		case 'Z': 
			...
			break; 
		case 'Y': 
			strncpy(str, hv_poinfo.pwd, bll);
			break;
		case 'O': 
		/*add by wangyongwei 增加合计数*/
			vtcp_log("[%s][%d]开始打印--交易金额!\n",__FILE__,__LINE__);
			sprintf( fmt, "%%%d.2lf", bll ); 
