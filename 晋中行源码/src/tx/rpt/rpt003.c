...
int rpt003()
{
	...
	/*XRB
	if( vparm[0]=='2' )
		sprintf(filename,"%s/readrfc_nm1.txt",pathname);
	else if( vparm[0]=='1' )
		sprintf(filename,"%s/readrfc_nm2.txt",pathname);
	else
	*/
		sprintf(filename,"%s/readrfc.txt",pathname);
	...
	if( fp == NULL )
	{
		printf("open file readrfc.txt error!\n");
		goto ErrExit;
	}
