...
rpt928()
{
	...
	strcpy( filename , "RPT928");
	strcpy(vpfm, "RPT928");
	
	/**组成查询**/
	memset(wherelist,'\0',sizeof(wherelist));
	strcat(wherelist," abs(ys_bal-bal)>0.001 and ac_type<>'5'  order by opn_br_no ") ;
...
char str[200];
{
	...
	{ 
		...
		case 'N': /*余额*/
			...
			break;	
		case 'O': /*积数*/
			sprintf(t_str,"%.2lf",gDd_mst.intst_acm);
			sprintf(fmt,"%%-%ds",bll);
			sprintf(str,fmt,t_str);
			break;
		case 'L':  /*户      名 */
