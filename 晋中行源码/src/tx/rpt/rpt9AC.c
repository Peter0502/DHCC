...
OkExit:
	...
	sprintf(acErrMsg,"导入活期/贷款/内部明细成功![%s]",g_pub_tx.reply);
	WRITEMSG
...
ErrExit:
	...
	sprintf(acErrMsg,"导入活期/贷款/内部明细失败![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 1;	
} 
...
int insert_dd_hst()
{
	...
	*/
	ret = Dd_mst_hst_Dec_Sel(g_pub_tx.reply,"tx_date>%ld and tx_date<=%ld and tx_code<>'2611' order by ac_id,tx_date,hst_cnt",beg_tx_date,end_tx_date);
	ERR_DEAL
...
int insert_in_hst()
{
	...
	while(1)
	{
		...
		if(pre_ac_id!=s_in_mst_hst.ac_id)
		{
			...
			/*** 查看科目借贷属性 com_item.dc_ind  ***/
			memset(&s_in_parm,0x00,sizeof(s_in_parm));
			memset(&s_com_item,0x00,sizeof(s_com_item));
			ret = In_parm_Sel(g_pub_tx.reply,&s_in_parm,"prdt_no='%s'",s_in_mst.prdt_no);
			ERR_DEAL
			
			ret = Com_item_Sel(g_pub_tx.reply,&s_com_item,"acc_hrt='%s'",s_in_parm.acc_hrt);
			ERR_DEAL
			printf("[%s][%d]内部帐户产品[%s]科目为acc_hrt[%s]借贷为[%s]\n",__FILE__,__LINE__,s_in_mst.prdt_no,s_in_parm.acc_hrt,s_com_item.dc_ind);
		}
		...
		if( s_com_item.dc_ind[0]=='2' )/* 1：借 资产；2：贷 负债*/
			{
		    if(s_in_mst_hst.add_ind[0]=='0')	/*0 表示借方0：减少；1：增加**/
		    	memcpy(s_acno_hst_log.dc_ind,"1",sizeof(s_acno_hst_log.dc_ind)-1);
...
int insert_ln_hst()
{
	...
	while(1)
	{
		...
		if(s_ln_mst_hst.add_ind[0]=='1')	/*貸款1 表示借方**/
			strcpy(s_acno_hst_log.dc_ind,"1");
		else if(s_ln_mst_hst.add_ind[0]=='0')	/*貸款0 表示贷方**/
			strcpy(s_acno_hst_log.dc_ind,"2");
