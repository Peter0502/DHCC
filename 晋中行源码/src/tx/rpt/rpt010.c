...
FILE    *fp; 
int ret = 0;
double dr_sum=0.00;/*借方合计*/
double cr_sum=0.00;/*贷方*/
long dr_cnt=0;
long cr_cnt=0;

int get_ratio_tdbdt(int bli,int bll,char str[100]);
...
rpt010()
{
	...
	/*strcpy( filename , "BRtdbdt" );	
	strcpy(vpfm, "tdbdt" );*/
	strcpy( filename , "RPT010" );	
	strcpy(vpfm, "RPT010" );
	...
	/*--------- add by jack 20031102 --for 将批量和日终都去掉-----*/
	memset( tmpstr, 0x0, sizeof(tmpstr) );
	/*strcpy( tmpstr, "tx_code != '6604' and tx_code !='9001' and " );*/
	strcat( wherelist,tmpstr );
	/*---------- add by jack end ------------*/

	/**组成查询**/
	
	strcat(wherelist," sts='0' and amt!=0  ORDER BY tx_tx_br_no , tx_opn_br_no , trace_no , trace_cnt ") ;
	...
	while(1)
	{
		...
		/* 机构变换时 */
		pub_base_strpack( s_trace_log_bk.tx_tx_br_no );
		if( strcmp( s_trace_log_bk.tx_tx_br_no , old_br_no ) )
		{
			/* 第一个机构 */
			if( strcmp( old_br_no , "*****" ) )
			{
			...
			strcpy( old_br_no , s_trace_log_bk.tx_tx_br_no );
			
			page=1;
			line_file=1;
			
			/* 删除原文件 */
			ret = pub_rpt_rmfile( s_trace_log_bk.tx_tx_br_no , filename , 0 );
			ERR_DEAL
			/* 打开文件 */	
			ret = pub_rpt_openfile( &fp , s_trace_log_bk.tx_tx_br_no , filename );
			ERR_DEAL
			/* 打印页首 */
			ret = print_head();
			ERR_DEAL
...
OkExit:
	...
	sprintf(acErrMsg,"生成通存通兑流水清单(本代他)成功!!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 0;
ErrExit:
	sql_rollback();	/*--事务回滚--*/
	sprintf(acErrMsg,"生成通存通兑流水清单(本代他)失败!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 1;
}
...
{	
	ret = pub_rpt_read_pfm_qd(fp,&line,vpfm,"0001",vrec,&opncnt,get_ratio_tdbdt);
	ERR_DEAL
...
/* 赋值函数 */
int get_ratio_tdbdt( bli,bll,str )
int bli,bll;
char str[100];
{ 
	char vhm[101],ss_add_ind[3]; 
	char vstr[101]; 
	char l_kmm[31];
	char amt_tmp[21]; 
	char tmp_inf[21];
	...
	{ 		
		...
		s_trace_log_bk.tx_tx_br_no);
			...
			sprintf( t_str , "%s    ( %5s )" , sCom_branch.br_name , s_trace_log_bk.tx_tx_br_no); 
			sprintf(fmt, "%%-%ds", bll);
			sprintf(str, fmt, t_str);
			break;
		case 'D': /* 页码 */
