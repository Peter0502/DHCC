...
int get_ratio_lcmxhz(int bli,int bll,char str[200]);
int rptLC5()
{
	char filename[50];
	char wherelist[1024];
	...
	struct fn_parm_c sFn_parm1;
	
	memset(&sFn_parm,0x00,sizeof(sFn_parm));
	memset(&sFn_parm1,0x00,sizeof(sFn_parm1));	
	...
	memset(cPrdtlist,0x00,sizeof(cPrdtlist));
	...
	ret = Fn_parm_Dec_Sel(g_pub_tx.reply,"end_date >= %ld and stat='1'",g_pub_tx.tx_date);
	if(ret&&ret!=100)
	{
	...
	while(1)
	{
		ret = Fn_parm_Fet_Sel(&sFn_parm1,g_pub_tx.reply);
		if(ret == 100 )
		{
				break;
		}
		...
		strcat(cPrdtlist,"'");	
		strcat(cPrdtlist,sFn_parm1.prdt_code);
		strcat(cPrdtlist,"',");	
	}
	if(strlen(cPrdtlist))
	cPrdtlist[strlen(cPrdtlist)-1] = '';
	else
	{
		sprintf(cPrdtlist,"''");	
	}
	...
	sprintf(wherelist,"prdt_code in (%s) and sts = '1' order by prdt_code,tx_br_no,tx_date,pact_no ",cPrdtlist);
...
{	
	ret=pub_rpt_read_pfm_qd(fp,&line,vpfm,"0001",vrec,&opncnt,get_ratio_lcmxhz);
	ERR_DEAL
...
/* 赋值函数 */
int get_ratio_lcmxhz( bli,bll,str )
int bli,bll;
char str[200];
{
	char fmt[10]; 
    ...
    case 'R':/*起息日期*/
	  ...
	  case 'S':/*到期日期*/
		...
		case 'G':	/*认购渠道*/
			...
			if(strcmp(sFn_reg.chnl_type,"0300") == 0)
			{
