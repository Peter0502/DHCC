...
int get_ratio_tdb(int bli,int bll,char str[100]);
int get_ratio_tdb1(int bli,int bll,char str[100]);
...
rpt023()
{
	...
	if (ret != 0)
	{
		sprintf(acErrMsg, "生成储蓄异地通兑他代本机构流水帐!!");
		WRITEMSG
		goto ErrExit;
	}
...
OkExit:
	...
	sprintf(acErrMsg,"生成储蓄异地通兑他代本机构流水帐处理成功![%s]",g_pub_tx.reply);
    WRITEMSG
...
ErrExit:
    ...
    sql_rollback(); /*--事务回滚--*/
	sprintf(acErrMsg,"生成储蓄异地通兑他代本机构流水帐处理失败![%s]",g_pub_tx.reply);
    WRITEMSG
    set_zd_data("0120",g_pub_tx.reply);
    return 1;
}
...
int cxyd_tdb_list()
{
	...
	while(1)
	{
		...
		ret = Chnl_self_trade_Dec_Sel( g_pub_tx.reply, "tx_date=%ld and \
			tx_br_no = '%s' and (tx_code = '6301' or (tx_code = '6203' and \
			info_source = '1') ) and stat = '0'", g_pub_tx.tx_date, \
			sCom_branch.br_no );
		if ( ret ) 
		{
		...
		ret = Four_tradelog_Dec_Sel( g_pub_tx.reply, "tx_date=%ld and \
			sl_bank = '%s' and tx_code in ('7801','7802') and tx_stat = '0'", \
		...
		while(1)
		{
			...
			if ( strcmp( sFour_tradelog.tx_code, "7801" ) == 0 )
			{
...
int get_ratio_tdb1( bli, bll, str )
int bli,bll;
char str[100];
{
	char vhm[101]; 
