...
/*		ret = Gl_sub_Fet_Sel( &s_gl_sub,g_pub_tx.reply );*/
	...
	if(!memcmp(s_gl_sub.br_no,"59801",5))
	{
		printf("[%s][%d]进入计财部的科目![%s][%c]\n",__FILE__,__LINE__,s_gl_sub.acc_hrt , old_acc_head[0]);
	  ...
	  /***************add by liuyue 20061202 *****//***如果科目号到了6  但是 6，7 都没有发生额，打印出表表内的合计就break***/
	  /*
	  if(s_gl_sub.acc_hrt[0]=='6') 
	  {
	  		 ret=Gl_sub_Sel( g_pub_tx.reply , &sGl_sub , "br_no='%s' and cur_no='01' and ( acc_hrt like '6%%' or acc_hrt like '7%%' ) \
			     and (mdr_bal<>0 or mcr_bal<>0 or dr_bal<>0 or cr_bal<>0 or mdr_amt<>0 or mcr_amt<>0)" ,s_gl_sub.br_no );
			     if( ret==100 ) 
			      { 
			      	  tag = 1 ;
			      	  ret = print_tail_A( ); 
	  ...
	  if( !pub_base_CompDblVal(s_gl_sub.mdr_bal,0.00) &&
			  !pub_base_CompDblVal(s_gl_sub.mcr_bal,0.00) &&
		  	!pub_base_CompDblVal(s_gl_sub.dr_bal,0.00) &&
		  	!pub_base_CompDblVal(s_gl_sub.cr_bal,0.00) &&
		  	!pub_base_CompDblVal(s_gl_sub.mdr_amt,0.00) &&
		  	!pub_base_CompDblVal(s_gl_sub.mcr_amt,0.00)  )
			/*{
			       	   continue;
			}*/
			{
			  ...
			  pub_base_strpack( s_com_item.acc_no ); 
				...
				if( strlen(s_com_item.acc_no)==7 )
				{
			        continue;
			       	...
			       	and (mdr_bal<>0 or mcr_bal<>0 or dr_bal<>0 or cr_bal<>0 or mdr_amt<>0 or mcr_amt<>0)" ,s_gl_sub.br_no, s_com_item.acc_no);
			       	 if( ret==100 ) 
			       	 {
			       	   continue;
			         }
	...
	if(!memcmp(s_gl_sub.br_no,"59801",5))
	{
		printf("[%s][%d]注意了进入计财部了![%c][%c]\n",__FILE__,__LINE__,s_gl_sub.acc_hrt[0] , old_acc_head[0]);
		...
		if( s_gl_sub.acc_hrt[0] != old_acc_head[0] )
		{
			if( old_acc_head[0] == 'A' )				/* 资产类 */
			{
				...
				/*** add by martin 科目無餘額***/
			 ret=Gl_sub_Sel( g_pub_tx.reply , &sGl_sub , "br_no='%s' and cur_no='01' and acc_hrt like '%s%%' \
			     and (mdr_bal<>0 or mcr_bal<>0 or dr_bal<>0 or cr_bal<>0 or mdr_amt<>0 or mcr_amt<>0)" ,s_gl_sub.br_no, acc_type );
			     if( ret==100 ) 
			      { 
			       	   strcpy( acc_type , "2" );
			       	   old_acc_head[0] = '2';
			...
			else 
			{
				...
				{
					if(!memcmp(s_gl_sub.br_no,"59801",5))
						printf("[%s][%d]old_acc_head==[%c]\n",__FILE__,__LINE__,old_acc_head[0]);
			...
			{
				page++;
				ret = print_head_C( ); /**表外头部**/
				head_flag ++;
			  ERR_DEAL 
		  } else if(tag != 2){         /* 表外不打汇总 */
			 	 ret = print_body_A( ); /**合计值部分***/
			   ERR_DEAL
			   line_file++;
			}
    ...
    loop_cnt++; /** aaa **/
		if( line_file>PAGE_CNT )			
		{
			...
			/* 打印页尾 */
			if( tag==0 )
				ret = print_tail_B(); /**表内打印表尾部***/
			else
				ret = print_tail_C(); /*表外打印表的尾部，封表***/
			ERR_DEAL
...
OkExit:
	...
	sprintf(acErrMsg,"支行月报表生成处理成功!!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 0;
ErrExit:
	sql_rollback();	/*--事务回滚--*/
	sprintf(acErrMsg,"支行月报表生成处理失败!!!!![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 1;
}
...
{ 
	...
	{ 
		...
		case 'E': /* 月初借余额 */
			...
			{
			  ret = sql_sum_double( "gl_sub","mdr_bal",&tmp_amt,"acc_hrt='40602' and br_no='%s' ",s_gl_sub.br_no);
			...
			sprintf( amt_tmp, "%.2lf" , s_gl_sub.mdr_bal + gl_sub_low.mdr_bal); 
			if( !pub_base_CompDblVal(atof(amt_tmp),0.00) )
			{
				memset(str,' ',bll );
				break;
		...
		case 'F': /* 月初贷余额 */
			...
			{
			  ret = sql_sum_double( "gl_sub","mcr_bal",&tmp_amt,"acc_hrt='40601' and br_no='%s' ",s_gl_sub.br_no);
			...
			sprintf( amt_tmp, "%.2lf" , s_gl_sub.mcr_bal + gl_sub_low.mcr_bal ); 
			if( !pub_base_CompDblVal(atof(amt_tmp),0.00) )
			{
				memset(str,' ',bll );
				break;
			...
			break;
		case 'G': /* 本月借发生额 */
			sprintf( amt_tmp, "%.2lf" , s_gl_sub.mdr_amt+ gl_sub_low.mdr_amt ); 
			if( !pub_base_CompDblVal(atof(amt_tmp),0.00) )
			{
				memset(str,' ',bll );
				break;
			...
			break;
		case 'H': /* 本月贷发生额 */
			sprintf( amt_tmp, "%.2lf" , s_gl_sub.mcr_amt + gl_sub_low.mcr_amt ); 
			if( !pub_base_CompDblVal(atof(amt_tmp),0.00) )
			{
				memset(str,' ',bll );
				break;
...
/*		case 'Y':  汇总 
		...
		case 'K': /* 月初借余额 */
			...
			if( flag==0 )
			{
				ret = sql_sum_double( "gl_sub","mdr_bal",&tmp_amt,"acc_hrt[1]='%c' and br_no in('10101','11901') and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) ", acc_type[0] );
			 /* if( acc_type[0]=='4' )
			  {
			  ret = sql_sum_double( "gl_sub","mdr_bal",&tmp_amt1,"acc_hrt='40602' and br_no='%s' ",s_gl_sub.br_no);
				tmp_amt+=tmp_amt1;
			  }*/
			}
			else 
			{
				ret = sql_sum_double( "gl_sub","mdr_bal",&tmp_amt,"acc_hrt[1]>='1' and br_no in('10101','11901') and acc_hrt[1]<='5' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) " );
			  /*ret = sql_sum_double( "gl_sub","mdr_bal",&tmp_amt1,"acc_hrt='40602' and br_no='%s' ",s_gl_sub.br_no);
				tmp_amt+=tmp_amt1;*/
			}
			ERR_DEAL
		...
		case 'L': /* 月初贷余额 */
			if( flag==0 )
			{
				/*ret = sql_sum_double( "gl_sub","mcr_bal",&tmp_amt,"substr(acc_hrt,1,1)='%c' and br_no='%s' and acc_hrt in ( select acc_hrt from com_item where acc_lvl='1' and acc_hrt !='40600')  ", acc_type[0] ,s_gl_sub.br_no);*/
			  ret = sql_sum_double( "gl_sub","mcr_bal",&tmp_amt,"substr(acc_hrt,1,1)='%c' and br_no in('10101','11901') and acc_hrt in ( select acc_hrt from com_item where acc_lvl='1' )  ", acc_type[0] );
			  /*if(acc_type[0]=='4')
			  {
			  ret = sql_sum_double( "gl_sub","mcr_bal",&tmp_amt1,"acc_hrt='40601' and br_no='%s' ",s_gl_sub.br_no);
				tmp_amt+=tmp_amt1;
			  }*/
			}
			else 
			{
				/*ret = sql_sum_double( "gl_sub","mcr_bal",&tmp_amt,"substr(acc_hrt,1,1)>='1' and br_no='%s' and substr(acc_hrt,1,1)<='5' and acc_hrt in ( select acc_hrt from com_item where acc_lvl='1' and acc_hrt!='40600' ) ",s_gl_sub.br_no );*/
			  ret = sql_sum_double( "gl_sub","mcr_bal",&tmp_amt,"substr(acc_hrt,1,1)>='1' and br_no in('10101','11901') and substr(acc_hrt,1,1)<='5' and acc_hrt in ( select acc_hrt from com_item where acc_lvl='1' ) " );
			  /*ret = sql_sum_double( "gl_sub","mcr_bal",&tmp_amt1,"acc_hrt='40601' and br_no='%s' ",s_gl_sub.br_no);
				tmp_amt+=tmp_amt1;*/
			}
			ERR_DEAL
		...
		case 'O': /* 本月借发生额 */
			if( flag==0  )
			{
				ret = sql_sum_double( "gl_sub","mdr_amt",&tmp_amt,"acc_hrt[1]='%c' and br_no in('10101','11901') and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) ", acc_type[0] );
			}
			else
			{
				ret = sql_sum_double( "gl_sub","mdr_amt",&tmp_amt,"acc_hrt[1]>='1' and br_no in('10101','11901') and acc_hrt[1]<='5' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) " );
			}
			ERR_DEAL
		...
		case 'P': /* 本月贷发生额 */
			if( flag==0 )
			{
				ret = sql_sum_double( "gl_sub","mcr_amt",&tmp_amt,"acc_hrt[1]='%c' and br_no in('10101','11901') and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' )", acc_type[0] );
			}
			else
			{
				ret = sql_sum_double( "gl_sub","mcr_amt",&tmp_amt,"acc_hrt[1]>='1' and br_no in('10101','11901') and acc_hrt[1]<='5' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) " );
			}
			ERR_DEAL
		...
		case 'V': /* 月末贷余额 */
			if( flag==0 )
			{
			 ...
			 ret = sql_sum_double( "gl_sub","cr_bal",&tmp_amt,"acc_hrt[1]='%c' and br_no in('10101','11901') and acc_hrt in ( select acc_hrt from com_item where acc_lvl='1' )", acc_type[0] );
			 /* if(acc_type[0]=='4')
			  {
			...
			else
			{
			  ...
			  ret = sql_sum_double( "gl_sub","cr_bal",&tmp_amt,"acc_hrt[1]>='1'  and br_no in('10101','11901') and acc_hrt[1]<='5' and acc_hrt in ( select acc_hrt from com_item where acc_lvl='1' ) " );
			  /*ret = sql_sum_double( "gl_sub","cr_bal",&tmp_amt1,"acc_hrt='40601' and br_no='%s' ",s_gl_sub.br_no);
				tmp_amt+=tmp_amt1;*/
			}
			ERR_DEAL
			...
			break;
	 case 'Y': /* 月初余额 */
			sprintf( amt_tmp, "%.2lf" , s_gl_sub.mdr_bal+s_gl_sub.mcr_bal + gl_sub_low.mdr_bal + gl_sub_low.mcr_bal); 
			if( !pub_base_CompDblVal(atof(amt_tmp),0.00) )
			{
				memset(str,' ',bll );
				break;
