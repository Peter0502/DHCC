...
rptHP2()
{
	...
	g_reply_int = Hv_poinfo_Dec_Sel(g_pub_tx.reply, \
		"l_tx_date=%ld  and po_sts='2' ORDER BY br_no  ",sComSysParm.lst_date );
	if( g_reply_int ) 
	{ 
	...
	{ 
		...
		sum_amt+=hv_poinfo.po_bal;/**add by wangyongwei*20071010*/
    ...
    E-挂失 F-解挂失 G-逾期未用退回 H-未用解 I-未用冲帐冲帐             	*/
		...
		case 'Y': 
			...
			*
			...
			break; 
			********************/
		/**add by wangyongwei 合计金额 20071010**/
		case 'O':
			vtcp_log("[%s][%d]开始打印--合计金额!\n",__FILE__,__LINE__);
			sprintf( fmt, "%%%d.2lf", bll ); 
