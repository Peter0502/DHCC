...
rptHP3()
{
	...
	g_reply_int = Hv_poinfo_Dec_Sel(g_pub_tx.reply, \
		"pay_date=%ld ORDER BY br_no  ",sComSysParm.lst_date );
	if( g_reply_int ) 
	{ 
	...
	{ 
		...
		else if( g_reply_int == 100  )
		{
			...
			if(iCycNum!=0)   /**只有在有记录的时候才会在下面的代码中打开文件的****/
			{
					...
					memcpy(&hv_poinfo, &sum_hv_poinfo, sizeof(hv_poinfo));
					vtcp_log("%s,%d 剩余金额rem_amt[%lf],cash_amt[%lf]\n",__FILE__,__LINE__,hv_poinfo.rem_amt,hv_poinfo.cash_amt);
		...
		if( strcmp( hv_poinfo.br_no, pre_br_no ) )
		{
			...
			memset(&sum_hv_poinfo, 0x0, sizeof(sum_hv_poinfo));
		...
		sum_hv_poinfo.po_bal+=hv_poinfo.po_bal;
		sum_hv_poinfo.cash_amt+=hv_poinfo.cash_amt;
		sum_hv_poinfo.rem_amt+=hv_poinfo.rem_amt;
		vtcp_log("%s,%d 剩余金额rem_amt[%lf],cash_amt[%lf]\n",__FILE__,__LINE__,sum_hv_poinfo.rem_amt,sum_hv_poinfo.cash_amt);
		...
		if(0 != rownum &&  (rownum+1) % LINE_CNT == 0 )
		{
