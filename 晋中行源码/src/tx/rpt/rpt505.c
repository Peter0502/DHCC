...
vtcp_log("[%s][%d]开始打印大额往帐清单!\n",__FILE__,__LINE__);
	...
	g_reply_int = Hv_zf_Dec_Sel(g_pub_tx.reply, \
		"wt_date=%ld  and  lw_ind = '1' and hv_sts ='5'  ORDER BY br_no, cmtno,orderno,hv_sts  ",sComSysParm.lst_date );
	if( g_reply_int ) 
	{ 
	...
	{ 
		...
		if( strcmp( hv_zf.br_no, pre_br_no ) )
		{
			...
			/*** total = g_reply_int / 25 + 1; del by martin 070519  DD打2行+1=3***/
			total = g_reply_int / 14 + 1;
			hy_line = 0;
			page = 1;
		...
		/**if( hy_line % 25 == 0  &&hy_line!=iRecordNum ) del by martin 070519 3行一周期 **/
		if( hy_line % 14 == 0  &&hy_line!=iRecordNum )
		{
...
char mygsm[3];
{
	...
	vtcp_log("[%s][%d]505505505-------[%s][%d],mygsm===[%s]\n",__FILE__,__LINE__,str,bll,mygsm);
