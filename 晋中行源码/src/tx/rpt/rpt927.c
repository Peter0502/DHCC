...
rpt927()
{
	...
	strcpy( filename , "RPT927");
	strcpy(vpfm, "RPT927");
	
	/**组成查询**/
	
	strcat(wherelist," ac_id in (select ac_id from dd_mst where ac_type<>'5')  ORDER BY br_no,ac_no ") ;
...
{ 
	...
	memset(&sOd_parm,0x00,sizeof(struct od_parm_c));
		...
		case 'M': /*产品名称*/
			...
			switch(sPrdt_ac_id.ac_id_type[0])
			{
				...
				case '2':				
					...
					break;
				case '3':
					ret=Ln_parm_Sel(g_pub_tx.reply,&sLn_parm,"prdt_no='%s'",sPrdt_ac_id.prdt_no);
					if(ret)
					{
					...
					pub_base_strpack(sLn_parm.title);
					sprintf(t_str,"%s",sLn_parm.title);
					sprintf(fmt,"%%-%ds",bll);
					sprintf(str,fmt,t_str);
					break;
				case '4':
					ret=Od_parm_Sel(g_pub_tx.reply,&sOd_parm,"prdt_no='%s'",sPrdt_ac_id.prdt_no);
					if(ret)
					{
						sprintf(acErrMsg,"查询贷款产品参数表出错!![%d]",ret);
						WRITEMSG
					...
					pub_base_strpack(sOd_parm.title);
					sprintf(t_str,"%s",sOd_parm.title);
					sprintf(fmt,"%%-%ds",bll);
					sprintf(str,fmt,t_str);
					break;
				case '9':	
