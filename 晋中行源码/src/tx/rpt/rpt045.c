...
int get_ratio_lnqkkkhd(int bli,int bll,char str[100]);
...
rpt045()
{
	...
	pub_base_sysinit();
/*	
	strcpy( filename , "BRlnqkkkhd" );	
	strcpy(vpfm, "lnqkkkhd" );
*/	
	strcpy( filename , "RPT045");
	strcpy(vpfm, "RPT045");
...
{	
	ret = pub_rpt_read_pfm_qd(fp,&line,vpfm,"0001",vrec,&opncnt,get_ratio_lnqkkkhd);
	ERR_DEAL
...
/* 赋值函数 */
int get_ratio_lnqkkkhd( bli,bll,str )
int bli,bll;
char str[100];
{ 
	char vhm[101]; 
	...
	{ 
		...
		case 'R': /* 起始日期 */
			...
			sprintf( str , fmt , t_str );
			break;
		/***根据长治要求,改为交易当日, modify by wanglei 20070122 
		case 'S': * 终止日期 
