...
rpt990( )
{
   ...
   if( !pub_base_CompDblVal(s_gl_mst.dr_bal,0.00) &&        /* add by martin  start*/
	  ...
	  if(ret)
	  {
		...
		if(memcmp(old_br_no,s_gl_mst.br_no,sizeof(s_gl_mst.br_no)-1) )  
    {
    	if(strlen(old_br_no))
    	{
    		 tag = 0;
...
{ 
	...
	{ 
		...
		case 'D': /* 页码 */
			...
			break; 
		case 'E': /* 季初借余额 */
			sprintf( amt_tmp, "%.2lf" , s_gl_mst.qdr_bal ); 
			...
			break;
		case 'F': /* 季初贷余额 */
			sprintf( amt_tmp, "%.2lf" , s_gl_mst.qcr_bal ); 
			pub_rpt_flttomon( amt_tmp,amt_tmp );
			if( strlen(amt_tmp)>bll ) 
				sprintf( amt_tmp, "%.2lf" , s_gl_mst.qcr_bal ); 
			sprintf(fmt,"%%%ds",bll); 		
			...
			break;
		case 'G': /* 本季借发生额 */
			sprintf( amt_tmp, "%.2lf" , s_gl_mst.qdr_amt ); 
			pub_rpt_flttomon( amt_tmp,amt_tmp );
			if( strlen(amt_tmp)>bll ) 
				sprintf( amt_tmp, "%.2lf" , s_gl_mst.qdr_amt ); 
			sprintf(fmt,"%%%ds",bll); 		
			...
			break;
		case 'H': /* 本季贷发生额 */
			sprintf( amt_tmp, "%.2lf" , s_gl_mst.qcr_amt ); 
			pub_rpt_flttomon( amt_tmp,amt_tmp );
			if( strlen(amt_tmp)>bll ) 
				sprintf( amt_tmp, "%.2lf" , s_gl_mst.qcr_amt ); 
			sprintf(fmt,"%%%ds",bll); 		
...
/*		case 'Y':  汇总 
		...
		case 'K': /* 季初借余额 */
			...
			if( flag==0 )
			{
				ret = sql_sum_double( "gl_mst","qdr_bal",&tmp_amt,"acc_hrt[1]='%c' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) ", acc_type[0] );
			}
			else 
			{
				ret = sql_sum_double( "gl_mst","qdr_bal",&tmp_amt,"acc_hrt[1]>='1' and acc_hrt[1]<='5' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) " );
			}
			ERR_DEAL
		...
		case 'L': /* 季初贷余额 */
			if( flag==0 )
			{
				ret = sql_sum_double( "gl_mst","qcr_bal",&tmp_amt,"acc_hrt[1]='%c' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) ", acc_type[0] );
			}
			else 
			{
				ret = sql_sum_double( "gl_mst","qcr_bal",&tmp_amt,"acc_hrt[1]>='1' and acc_hrt[1]<='5' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) " );
			}
			ERR_DEAL
		...
		case 'O': /* 本季借发生额 */
			if( flag==0  )
			{
				ret = sql_sum_double( "gl_mst","qdr_amt",&tmp_amt,"acc_hrt[1]='%c'  and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) ", acc_type[0] );
			}
			else
			{
				ret = sql_sum_double( "gl_mst","qdr_amt",&tmp_amt,"acc_hrt[1]>='1' and acc_hrt[1]<='5' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) " );
			}
			ERR_DEAL
		...
		case 'P': /* 本季贷发生额 */
			if( flag==0 )
			{
				ret = sql_sum_double( "gl_mst","qcr_amt",&tmp_amt,"acc_hrt[1]='%c' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' )", acc_type[0] );
			}
			else
			{
				ret = sql_sum_double( "gl_mst","qcr_amt",&tmp_amt,"acc_hrt[1]>='1' and acc_hrt[1]<='5' and acc_hrt in ( select acc_hrt from com_item where acc_lvl[1]='1' ) " );
			}
			ERR_DEAL
		...
		case 'V': /* 季末贷余额 */
			...
			break;
		case 'Y': /* 季初余额 */
			sprintf( amt_tmp, "%.2lf" , s_gl_mst.qdr_bal+s_gl_mst.qcr_bal ); 
			pub_rpt_flttomon( amt_tmp,amt_tmp );
			sprintf(fmt,"%%%ds",bll); 		
			if( strlen(amt_tmp)>bll ) 
				sprintf( amt_tmp, "%.2lf" , s_gl_mst.qdr_bal+s_gl_mst.qcr_bal ); 
