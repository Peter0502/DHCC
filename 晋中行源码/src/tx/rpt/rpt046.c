...
int get_ratio_lnloqd(int bli,int bll,char str[100]);
...
rpt046()
{
  ...
  pub_base_deadlineD( g_pub_tx.tx_date, -1 ,&qry_date );
	...
	pub_base_sysinit();
/*
	strcpy( filename , "BRlnloqd" );	
	strcpy(vpfm, "lnloqd" );
*/
	strcpy( filename , "RPT046");
	strcpy(vpfm, "RPT046");
	/**组成查询**/	
	strcpy(wherelist," (in_lo_intst>0.00 or out_lo_intst>0.00 or cmpd_lo_intst>0.00) and repay_type not in ('3','4') and 1=1 ORDER BY opn_br_no,prdt_no,ac_id");
...
{	
	ret=pub_rpt_read_pfm_qd(fp,&line,vpfm,"0001",vrec,&opncnt,get_ratio_lnloqd);
	ERR_DEAL
...
/* 赋值函数 */
int get_ratio_lnloqd( bli,bll,str )
int bli,bll;
char str[100];
{ 
	char vhm[101]; 
