...
FILE *fp; 
int ret = 0;
static double tddrbal=0.00,tdcrbal=0.00,mdrbal=0.00,mcrbal=0.00;
...
rpt93a( )
{
  ...
  memset(old_acc, 0 ,sizeof(old_acc));
	...
	strcpy( filename , "RPT93a" );
	strcpy( vpfm, "RPT93a");
	ts=time(NULL); 

	ret = Gl_mst_hst_Dec_Sel( g_pub_tx.reply, "cur_no='01' and substr(acc_hrt,1,1)>='1'  and br_no='10101'  \
	and acc_hrt in ( select acc_hrt from com_item where acc_lvl='1' ) and substr(\"date\",5,2) = '%02d' ORDER BY br_no,acc_hrt,date",g_pub_tx.tx_date%10000/100 );
	ERR_DEAL
    ...
    if(memcmp(old_br_no,s_gl_mst_hst.br_no,sizeof(s_gl_mst_hst.br_no)-1) )
    {
    	if(strlen(old_br_no))
    	{
	       ...
	       ret = print_tail( );
			...
			page = 1;
			line_file = 0;
			tddrbal=0.00,tdcrbal=0.00,mdrbal=0.00,mcrbal=0.00;
    ...
    /* 打印科目信息 */
		...
		tddrbal+= s_gl_mst_hst.rdd_amt;
		tdcrbal+= s_gl_mst_hst.rcd_amt;
		mdrbal+=  s_gl_mst_hst.rdd_amt;
		mcrbal+=  s_gl_mst_hst.rcd_amt;
...
*/
		...
		case 'C': /* 机构名称 */
			...
			sprintf( t_str , "%5s 行名:%s 汇总" , s_gl_sub.br_no, br_name ); 
			sprintf(fmt, "%%-%ds", bll);
			sprintf(str, fmt, t_str);
			break;
		case 'D': /* 页码 */
			...
			break; 
		case 'E':/* 上年底借余额 */
			if( s_gl_sub.ydr_bal==0 && gl_sub_low.ydr_bal==0)
			{
				memset( str,' ',bll );
			}else
			{
			...
			break;
		case 'F': /* 上年底贷余额 */
			if( s_gl_sub.ycr_bal==0 && gl_sub_low.ycr_bal==0)
			{
				memset( str,' ',bll );
			}else
			{
		...
		case 'H': /* 止上月底贷发生额 */
			...
			break; 
		case 'J': /* 本年累计借发生额 */
			if( s_gl_sub.ydr_amt==0 && gl_sub_low.ydr_amt==0)
			{
				memset( str,' ',bll );
			}else
			{
			...
			break;	
		case 'K': /* 本年累计贷发生额 */	
			if( s_gl_sub.ycr_amt==0 &&  gl_sub_low.ycr_amt==0)
			{
				memset( str,' ',bll );
			}else
			{
			...
			break;
		case 'M': /* 上月底借余额 */
			if( s_gl_sub.mdr_bal==0 && gl_sub_low.mdr_bal==0)
			{
				memset( str,' ',bll );
			}else
			{
			...
			break;	
		case 'N': /* 上月底贷余额 */	
			if( s_gl_sub.mcr_bal==0 && gl_sub_low.mcr_bal==0)
			{
				memset( str,' ',bll );
			}else
			{
		...
		case 'O': /* 本日借发生额 */
			if( s_gl_mst_hst.rdd_amt==0 && gl_sub_low.rdd_amt==0 )
			{
				memset( str,' ',bll );
			}else
			{
			sprintf( amt_tmp, "%.2lf" , s_gl_mst_hst.rdd_amt + gl_sub_low.rdd_amt); 
			pub_rpt_flttomon( amt_tmp,amt_tmp );
			sprintf(fmt,"%%%ds",bll); 		
			if( strlen(amt_tmp)>bll ) 
				sprintf( amt_tmp, "%.2lf" , s_gl_mst_hst.rdd_amt + gl_sub_low.rdd_amt); 
			...
			break;
		case 'P': /* 本日贷发生额 */
			if( s_gl_mst_hst.rcd_amt==0 &&  gl_sub_low.rcd_amt==0 )
			{
				memset( str,' ',bll );
			}else
			{
			sprintf( amt_tmp, "%.2lf" , s_gl_mst_hst.rcd_amt + gl_sub_low.rcd_amt); 
			pub_rpt_flttomon( amt_tmp,amt_tmp );
			if( strlen(amt_tmp)>bll ) 
				sprintf( amt_tmp, "%.2lf" , s_gl_mst_hst.rcd_amt + gl_sub_low.rcd_amt); 
			sprintf(fmt,"%%%ds",bll); 		
			...
			break;
		case 'Q': /* 本日借余额 */
			if( s_gl_mst_hst.dr_bal==0 &&  gl_sub_low.dr_bal==0 )
			{
				memset( str,' ',bll );
			}else
			{
			sprintf( amt_tmp, "%.2lf" , s_gl_mst_hst.dr_bal + gl_sub_low.dr_bal); 
			pub_rpt_flttomon( amt_tmp,amt_tmp );
			if( strlen(amt_tmp)>bll ) 
				sprintf( amt_tmp, "%.2lf" , s_gl_mst_hst.dr_bal + gl_sub_low.dr_bal); 
			sprintf(fmt,"%%%ds",bll); 		
			...
			break;
		case 'R': /* 本日贷余额 */
			if( s_gl_mst_hst.cr_bal==0 &&  gl_sub_low.cr_bal==0 )
			{
				memset( str,' ',bll );
			}else
			{
			sprintf( amt_tmp, "%.2lf" , s_gl_mst_hst.cr_bal + gl_sub_low.cr_bal); 
			pub_rpt_flttomon( amt_tmp,amt_tmp );
			if( strlen(amt_tmp)>bll ) 
				sprintf( amt_tmp, "%.2lf" , s_gl_mst_hst.cr_bal + gl_sub_low.cr_bal); 
			sprintf(fmt,"%%%ds",bll); 		
