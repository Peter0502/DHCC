...
rpt520()
{
	...
	g_reply_int = Hv_fzf_Dec_Sel(g_pub_tx.reply, \
		"wt_date=%ld   and cmtno in ('319','320','321','322','323','324','325','327','328')  and otype ='2' ORDER BY br_no, cmtno,orderno,hv_fzf_sts  ",sComSysParm.lst_date );
	if( g_reply_int ) 
	{ 
...
char str[100];
{
	...
	switch( bli ) 
	{
		...
		case 'V':
			...
			memset(cTx_Desc,0x00,sizeof(cTx_Desc));
			cGetLvzfStat(hv_fzf.hv_fzf_sts[0],cTx_Desc);
		...
		case 'Z':
			vtcp_log("[%s][%d]开始打印--交易状态!\n",__FILE__,__LINE__);
			memset(cTx_Desc,0x00,sizeof(cTx_Desc));
			cGetLvCmnoDes(hv_fzf.cmtno,cTx_Desc);
			cDeal_str(cTx_Desc, bll);
			memcpy(str,cTx_Desc,bll);
			break;
		default : 
