...
spR001()
{
	...
	vtcp_log(" 日期 [%s]g_pub_tx.tx_date[%ld] ",tx_date,g_pub_tx.tx_date);
	...
	while(1)
	{
		...
		sPay_agent.tram = ys_amt;	/*交易金额*/
		strcpy(sPay_agent.remark, sAg_peritemmap.remark );	/*用户号*/
		strcpy(sPay_agent.sata,"1");	/*-- 需要批量处理 --*/
		strcpy(sPay_agent.result,"1");	/*-- 未处理 --*/
		strcpy(sPay_agent.cnt,"1");/*--代收付次数--*/
		strcpy(sPay_agent.result_name,fldstr[3]);/*--保存实应收金额--*/
		ret = 0;
