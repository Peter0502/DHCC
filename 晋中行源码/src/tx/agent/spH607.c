...
spH607()
{
	...
	if (ret != 0)
	{
		sprintf(acErrMsg,"修改个人编号密码错1[%s]",per_no);
		WRITEMSG
	...
	if (ret != 0)
	{
		sprintf(acErrMsg,"修改个人编号错密码2[%s]",per_no);
		WRITEMSG
		strcpy(g_pub_tx.reply,"H005");
		goto ErrExit;
	}
	...
	if (ret != 0)
	{
		sprintf(acErrMsg,"修改个人编号错密码3[%s]",per_no);
		WRITEMSG
		strcpy(g_pub_tx.reply,"H005");
		goto ErrExit;
	}
...
OkExit:
	...
	sprintf(acErrMsg,"修改个人编号密码成功![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 0;
ErrExit:
	ret = Ag_peracctinfo_Clo_Upd( );
	sprintf(acErrMsg,"修改个人编号密码失败![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 1;
}
