	...
	*/
	...
	while(1)
	{
		...
		*/
		...
		/*瑞阳供热生成代扣成功文件*/
		else if (sAg_unitinfo.opr_attri[0]=='0' && memcmp(sAg_unitinfo.pay_type,"51",2)==0)
	    	{	
			memset(filename0,0x0,sizeof(filename0));
				...
				/*start 晋中瑞阳供热*/
	  			...
	  			{						
					/*生成流水*/
	  				...
	  				sprintf(RTrace_no,"%08ldP%011ld",sPay_agent.tx_date,sPay_agent.trace_no);
	  				/*银行流水|业务日期|单位编号|操作员号|用户号|业务年度|实应收金额*/					
					fprintf( fp0,"%20s|%08ld|%12s|%20s|%20s|%4ld|%012.f\r\n",\
				...
				if(sAg_unitinfo.opr_attri[0]=='0')
	  			{
				    	...
				    	else if(memcmp(sAg_unitinfo.pay_type,"43",2)==0)
					{
				    	...
				    	else if(memcmp(sAg_unitinfo.pay_type,"51",2)==0)
					{
