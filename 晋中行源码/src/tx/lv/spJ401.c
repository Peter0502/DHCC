...
static struct TIS6560019_GROUP {
 ...
 struct TOS6560019_GROUP {
	...
	struct TQINFO1 dqdj;
	struct TQINFO1 djth;
	struct TQINFO1 ptjjhz;
	struct TQINFO1 dqjjhz;
	struct TQINFO1 ssjjhz;
	struct TQINFO1 ssdjhz;
	struct TQINFO2 ptjj;
	struct TQINFO2 dqjj;
	struct TQINFO2 ssjj;
	struct TQINFO2 ssdj;
	struct TQINFO2 tyxx;
	struct TQINFO2 ssxx;
	struct TQINFO3 query;
	struct TQINFO3 free;
	struct TQINFO3 askback;	
	struct TQINFO3 askcancel;	
	struct TQINFO3 askzf;	
	struct TQINFO3 askcz;	
	struct TQINFO3 czmsg;	
...
int spJ401()
{
	...
	if((iRc=iQueryInfo1(PKGNO_PTDJ,&tos19.ptdj)))	return(-1);
	...
	vtcp_log("[%s][%d]tos19===[%s]\n",__FILE__,__LINE__,(char *)&tos19);
	
	memcpy(&fd123,&tos19,sizeof(tos19));
...
char * iQueryStatCnt(char *pkgno,int sendflag,char *statlist){
	...
	memset(cTable,0x00,sizeof(cTable));
	...
	{/****cmtno业务***/
		...
		{
			sprintf(cTmplist,"cmtno = '%.3s' and  ",pkgno);
			strcat(sql_condition,cTmplist);
		}
		else if(apatoi(pkgno,3)%2==0)/**应答类交易**/
		{
			sprintf(cTmplist,"cmtno = '%d' and res_orderno is not null and res_date <>0  and ",apatoi(pkgno)-1);
			strcat(sql_condition,cTmplist);
		}
	...
	if(sendflag)/***如果是往帐****/
		strcat(sql_condition,"  lw_ind = '1' and  ");
	else 
		strcat(sql_condition,"  lw_ind = '2' and ");
	...
	if(pkgno[0]=='0')/***是pkg报文的查询***/
	{
		...
		memcpy(cTable,"lv_pkgreg",9);
		
		 record_num = sql_count(cTable,sql_condition);
		vtcp_log("[%s][%d]funck!\n",__FILE__,__LINE__);
		vtcp_log("[%s][%d]funck!\n",__FILE__,__LINE__);
		vtcp_log("[%s][%d]funck!\n",__FILE__,__LINE__);
	}
	else 
	{
		...
		vtcp_log("[%s][%d]funck!\n",__FILE__,__LINE__);
		memcpy(cTable,"hv_fzf",6);
		record_num = sql_count(cTable,sql_condition);
	}
	vtcp_log("[%s][%d]liuyue\n",__FILE__,__LINE__);
	vtcp_log("[%s][%d]record_num==[%d]\n",__FILE__,__LINE__,record_num);
	
	if(record_num<=0) 
		memset(cRecordNum,' ',4);
	else 
		sprintf(cRecordNum,"%4d",record_num);
	return cRecordNum;
