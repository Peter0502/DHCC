...
int spJ411()
{
	...
	/**add by wangyongwei当前台退票理由为空时,后台取出来是0**/
     if(strcmp(cRcp8,"0")==0){
          memset(cRcp8,0,sizeof(cRcp8));
     }
     ...
     if (cOptype[0]=='0')
     {
          ...
          memcpy(cPackid   ,payin.F_packid ,sizeof(cPackid)-1);
          memcpy(cPack_date,payin.F_packday,sizeof(cPack_date)-1);
          set_zd_data("0290",cPackid);
          set_zd_data("0470",cPack_date);
          /****end by NEWBP6 *****/   
          /**提取审核结果**/
     ...
     if (!sModifyFlag)
     {
          ...
          {
     /****add by xyy 2007-4-19 3:37 退票理由检查，困的不行了懒办法吧, 位置不对挪到下面 wyw
          zip_space(cRcp8);
		...
		goto ErrExit;
         ...
         vtcp_log("[%s][%d]payin_hz.T_txnum=[%s]\n",__FILE__,__LINE__,payin_hz.T_txnum);
          ...
          if(getenv("BPTEST")!=NULL && strcmp(getenv("BPTEST"),"BP_TEST")==0)
          {
              ...
              {
                     memcpy(payin_hz.T_txnum,"30102",5);
                     memcpy(payin_hz.F_txnum,"30102",5);
              }
...
int tis81_to_payin()
{
     ...
     /****add by xyy 2007-4-19 3:37 退票理由检查，困的不行了，懒办法吧**/
          zip_space(pcReason);
          vtcp_log("[%s][%d] leng[%d][%s]***\n",__FILE__,__LINE__,strlen(pcReason),pcReason);
           if(strlen(pcReason)%2!=0)
           {
                vtcp_log("[%s][%d]您输入的退票理由代码长度不正确 \n",__FILE__,__LINE__);
             sprintf( acErrMsg,"您输入的退票理由代码长度不正确");
                strncpy(g_pub_tx.reply,"P046",4);
           ...
           for(i=0;i<strlen(pcReason);i++,i++)
           {
          	vtcp_log("[%s][%d]退票理由代码[%s]\n",__FILE__,__LINE__,pcReason);
                memset(cReason,0,sizeof(cReason));
                vtcp_log("[%s][%d]i==[%d]\n",__FILE__,__LINE__,i);
                memcpy(cReason,pcReason+i,sizeof(cReason)-1);
                vtcp_log("[%s][%d]cReason==[%s]\n",__FILE__,__LINE__,cReason);
                iRc = Lv_param_Sel(g_pub_tx.reply, &lv_param, " par_code like 'TPLY%%' and par_value='%s' ",cReason);
                if (iRc==100)
                {
          ...
          vtcp_log("[%s][%d]退票理由代码[%s]\n",__FILE__,__LINE__,pcReason);
	   /**不成功的回执不能包括00***/
	   if(memcmp(pcReason,"00",2)==0 || memcmp(pcReason+2,"00",2)==0 || memcmp(pcReason+4,"00",2)==0 || memcmp(pcReason+6,"00",2)==0){
		sprintf(acErrMsg,"退票理由中不能包括00!");
		set_zd_data(DC_GET_MSG,acErrMsg);
		WRITEMSG
		strcpy(g_pub_tx.reply,"D106");
		return(-1);
           ...
           a = apatoi(pcReason,2);
           b = apatoi(pcReason+2,2);
           c = apatoi(pcReason+4,2);
           d = apatoi(pcReason+6,2);
           
          if(strlen(pcReason)==4)
          {
               if(a == b )
               {
                      vtcp_log("%s,%d,您输入的退票理由代码不能重复!\n",__FILE__,__LINE__);
	...
	/**add by xyy 检查退票理由**/
          iRc=iLvSetAddit(&payin_hz,"TPL",pcReason,sizeof(pcReason)-1); 
          if (iRc)
          {
