...
int sp9109( )
{
	...
	strcat( filename, "/tel.txt" );

	sprintf(tmp_str, "rm -f %s", filename);
	system( tmp_str );
	memset(tmp_str, 0x0, sizeof(tmp_str));

	sprintf(acErrMsg, "强行签退机构号为:[%s], filename=[%s]", br_no, filename);
	WRITEMSG
	...
	if (ret > 0)
	{
		...
		if ( fp )
		{
			fprintf(fp, "该机构还有 [%d] 个柜员未签退:\n", ret);
			fprintf(fp, "----------------------------------------------------------\n");
			fprintf(fp, "柜员号 姓  名       原因\n");
			fclose(fp);
		}
	}

	/* 判断未签退柜员是否可以签退 */
	ret = Com_tel_Dec_Sel(g_pub_tx.reply, "csts in ('0', '1') and br_no='%s' \
		and lvl != '6' and substr(tel,1,1) != '9'", br_no);
	if (ret != 0)
	{
	...
	while(1)
	{
		...
		***/
		...
		sprintf(sTel_control.msg, "该柜员由于[%ld]未签退而被锁定!!",
			g_pub_tx.tx_date); 
	...
	/*修改柜员状态为签退*/ 
	vtcp_log("QQQQQQQQQQQQQQQQQQQQQQQQQQQ\n");
	ret=sql_execute("update com_tel set csts='2' where csts in ('0','1') \
		and br_no='%s' and lvl != '6' and substr(tel,1,1) != '9'", br_no); 
	if ( ret ) 
	{ 
		...
		vtcp_log("PPPPPPPPPPPPPPPPPPPPPPPPPQQ\n");
...
*/
  ...
  /** add by martin 增加判断取消流水 20071121**/
	...
	trace_no not in (select trace_no from trace_log where sts='1') ",g_pub_tx.tx_date,br_no);
	if( ret )
	{
...
OkExit:
	...
	sprintf(acErrMsg,"强行签退营业所成功 reply is[%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 0;
ErrExit:
	sprintf(acErrMsg,"强行签退营业所失败 reply is[%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 1;
}
