...
int spB706()
{
	...
	strcpy( sMoStokRebuyInpb.pact_no, sMo_stok_rebuy_prot.pact_no );    /*协议编号*/
    get_zd_double("0400", &sMoStokRebuyInpb.amt );   /*还款本金*/
	get_zd_double("0410",&sMoStokRebuyInpb.intst );    /*还利息金额*/
	...
	g_reply_int = sql_sum_double( "mo_stok_rebuy_inpb", "amt", &amt_rec, 
		"pact_no='%s'", sMoStokRebuyInpb.pact_no );
    if( g_reply_int )
    {
        sprintf(acErrMsg,"求已录入合计金额出错!![%d]",g_reply_int);
        WRITEMSG
        goto ErrExit;
    }
	vtcp_log("amt_rec[%lf]",amt_rec);

	sMo_stok_rebuy_prot.bal-=amt_rec;
    /*pub_base_strpack(sMo_stok_rebuy_prot.opt_acno);*/
    /* 检查证券回购借出账户余额(减去已录入的金额)和交易金额的大小 */
	vtcp_log("bal[%lf],amt[%lf]",sMo_stok_rebuy_prot.bal,sMoStokRebuyInpb.amt);
    if (pub_base_CompDblVal(sMo_stok_rebuy_prot.bal,sMoStokRebuyInpb.amt)<0 )
    {
	...
	/*登记证券回购借入还款录入登记簿*/
	g_reply_int = Mo_stok_rebuy_inpb_Ins( sMoStokRebuyInpb, g_pub_tx.reply );
	if( g_reply_int )
	{
        sprintf(acErrMsg,"登记证券回购借入还款录入登记簿出错[%d]!!", g_reply_int );
        WRITEMSG
        goto ErrExit;
	}
