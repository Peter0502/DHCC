...
int spB502()
{
	...
	*******/
	...
	pub_base_strpack(sMo_ibl_c1.prdt_no);
    strcpy(g_pub_tx.prdt_code,sMo_ibl_c1.prdt_no);
	sprintf(acErrMsg, "产品编码为:[%s]", g_pub_tx.prdt_code);
	WRITEMSG
    ...
    ********/
    ...
    strcpy(g_pub_tx.name, sMo_ibl_c1.op_name);
    g_pub_tx.rate = sMo_ibl_c1.rate;
    g_pub_tx.over_rate = sMo_ibl_c1.over_rate;
