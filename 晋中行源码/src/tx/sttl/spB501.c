...
int spB501()
{
	...
	****/
	/***     20060921 martin 从介质与账户关系表查出*本行*ac_id放入拆借登记簿    
 if (strlen(ac_no) > 9)
	{
    	ret = Mdm_ac_rel_Sel(g_pub_tx.reply,&sMdm_ac_rel_c,"ac_no ='%s'",ac_no);
    	if (ret != 0)
...
***/
	/* 从公共参数表中查询处产品号 */ 
	ret = Com_parm_Sel(g_pub_tx.reply,&com_parm_c,"parm_code='TYCR' \ 
			and parm_seqn = 1 "); 
	if (ret != 0) 
	{ 
		sprintf(acErrMsg,"查询公共参数表错误!! [%d]",ret); 
		WRITEMSG 
		strcpy(g_pub_tx.reply,"B175"); 
		goto ErrExit; 
	} 
	
	pub_base_strpack(com_parm_c.val); 
	strcpy(sMo_ibl_c.prdt_no,com_parm_c.val); 
	
	sprintf(acErrMsg,"同业拆入产品号为: [%s]",com_parm_c.val); 
	WRITEMSG
    ...
    switch( atoi(flag) )
    {
        ...
        case 1 :
            /* 修改 */
            ret = Mo_ibl_Dec_Upd(g_pub_tx.reply,"pact_no = '%s' and sign ='1'",
                  sMo_ibl_c.pact_no);
            if (ret != 0)
            {
	    ...
	    strcpy(sMo_ibl_c.rowid, sMo_ibl_c1.rowid);
            ...
            if ( ret != 0)
            {
                sprintf(acErrMsg,"修改拆借登记簿错误!! ret =[%s]",ret);
                WRITEMSG
                Mo_ibl_Clo_Upd();
        ...
        case 2 :
            ...
            sprintf(acErrMsg,"删除拆借登记簿中记录pass!! ");
            WRITEMSG
