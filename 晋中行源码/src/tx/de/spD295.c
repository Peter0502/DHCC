...
spD295()
{
	...
	get_zd_data("0860",g_mo_city.payer_br_no);			/*付款人开户行名*/
	...
	if(tmp_ind[0]=='1')									/*1-扣付款人钱*/
	{
		strcpy(g_pub_tx.ac_no,g_mo_city.payer_ac_no);	/*付款人帐号*/
		strcpy(g_pub_tx.add_ind,"0");					/*给收款人扣款*/
		g_pub_tx.tx_amt1 = g_mo_city.sttl_amt;			/*发生金额*/
		strcpy(g_mo_city.cd_ind,"1");					/*2-提入借*/
	}
	else if(tmp_ind[0]=='2')							/*2-给收款人帐户存款*/
	{
		strcpy(g_pub_tx.ac_no,g_mo_city.payee_ac_no);	/*收款人帐号*/
		strcpy(g_pub_tx.add_ind,"1");					/*给收款人存款*/
		g_pub_tx.tx_amt1 = g_mo_city.sttl_amt;			/*发生金额*/
		strcpy(g_mo_city.cd_ind,"2");					/*2-提入贷*/
	}
