...
int spD093()
{
    ...
    */
    ...
    strcpy(cFilename1,"kflcsyhd");    /* 理财收益回单 */
    ...
    */
    ...
    while(1)
    {
	    ...
	    /*add by lwb 晋中商行要求一年为365天*/ 
	    /*客户认购时间小于周期的不属于分配收益范围*/
	   				 
    	    if(lDays== 366)
	        {
	         	lDays = 365;
	         }
	         /*如果是随时可以进行分配收益的话，就不必要有分段利率，直接都是按照实际利率计算lpj20151102*/
        ...
        fprintf(fp1,"               收益率:%-9.6lf  收益:%-17.2lf\n",\
                        dFct_rate,dFct_intst);
        fprintf(fp1,"               入账账号:%s\n",cAc_no);
        fprintf(fp1,"               入账户名:%s\n",cName);
