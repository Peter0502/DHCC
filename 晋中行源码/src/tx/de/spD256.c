...
spD256()
{
    ...
    if (ret != 0)
    {
        ...
        set_zd_data("1215","2");
        set_zd_data("121A","网点提入借方记帐");

	/*** 登记交易流水 ***/
	strcpy(g_pub_tx.brf, "网点提入借方记帐"); 
...
OkExit:
	...
	sprintf(acErrMsg,"网点提入借方记帐处理成功![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 0;
ErrExit:
	sprintf(acErrMsg,"网点提入借方记帐处理失败![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 1;
}
