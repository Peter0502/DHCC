...
spD251()
{
	...
	ret = sql_count("com_parm", "parm_code='CZZH' and val='%s'",
		g_pub_tx.ac_no1);
	if ( ret < 0 )
	{
		sprintf(acErrMsg, "统计公共参数表异常 【%d】", ret);
		WRITEMSG
		goto ErrExit;
	}
	...
	if ( ret > 0 && flag[0] == '1')
	{
		sprintf(acErrMsg, "非税错误账号改成正确账号[%s][%s]",
			ac_no, g_pub_tx.ac_no1);
		WRITEMSG
	...
	get_zd_data("0680",subflag);
	if( subflag[0]=='1' )
	{
	if( cl_sub() ) goto ErrExit;
	}
OkExit:
...
cl_sub()
{
	...
	get_zd_data( "0320",g_pub_tx.ac_no );
		strcpy(g_pub_tx.add_ind,"1");
		strcpy(g_pub_tx.brf,"子帐号存款");
...
vtcp_log("ac_no1=[%s]",g_pub_tx.ac_no1);
	ret=pub_base_sub_ac_trance();
	if( ret )
	{
