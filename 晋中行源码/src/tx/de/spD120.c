...
-----下面这几个tx_sub_def应该有了 D214 可能没有 请查询数据库后确定---
...
insert into tx_sub_rel values ('4713', 'D202', '1', '2', '102K', '转销');
insert into tx_sub_rel values ('4713', 'D003', '1', '2', '102K', '转销');
*************************************************/
#include <stdio.h>
#define EXTERN
#include "public.h"
...
int spD120()
 {
    ...
    double dXh_amt = 0.00;/*销户金额 等于 账面余额*/
    ...
    vtcp_log("[%s][%d] 销户标志：[%s]",__FILE__,__LINE__,cXh_flag);
    /*根据账号找ac_id*/
    ...
    if(sPrdt_ac_id.ac_id_type[0] == '2') /*定期*/
    {
        ...
        */
        ...
        {
            sprintf(acErrMsg,"账号被全部冻结或者只进不出，不允许直接扣划，请检查！");
            WRITEMSG
    ...
    if(cKh_type[0] == '1')/*直接扣划*/
    {
        if(cXh_flag[0] == '1')
        {
            ...
            vtcp_log("[%s][%d] 开始销户！g_pub_tx.reply = [%s]",__FILE__,__LINE__,g_pub_tx.reply);
        }
    ...
    else if(cKh_type[0] == '2')/*冻结扣划*/
    {
        ...
        if(cXh_flag[0] == '1')
        {
            ...
            sMo_hold.kh_sum_cnt ++;
            */
            sMo_hold.hold_sts[0] == '1'; /*销户了 就解除该笔冻结 留着也没用了*/
            strcpy(g_pub_tx.reply,"0003");
        }
        else if(cXh_flag[0] == '0')
        {
            if(sMo_hold.hold_typ[0] == '3')/*部分冻结扣划*/
            {
                ...
                */
                ...
                {
                    Mo_hold_Clo_Upd();
                    sprintf(acErrMsg,"扣划金额[%f]与该笔部分冻结金额[%f]不符",dKh_amt,dSy_kh_amt);
                    WRITEMSG
    ...
    strcpy(g_pub_tx.brf,"扣划业务");/*此备注请勿改动*/
    ret=pub_ins_trace_log();
    if(ret)
    {
        vtcp_log("[%s][%d]登记交易流水出错[%d]",__FILE__,__LINE__,ret);
        sprintf(acErrMsg,"[%d]登记交易流水出错",__LINE__);
