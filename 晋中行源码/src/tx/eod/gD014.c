...
gD014()
{
  ...
  char tmp[3];
	...
	*/
	ret = Ag_unitinfo_Dec_Sel( g_pub_tx.reply , "unit_stat = '0' and pay_type='43' \
    ...
    sumtram=0.00;/**/
	  ...
	  {	
		...
		sprintf( filename1, "%s/zls/%s/BR1%s.txt", getenv("HOME"), sAg_unitinfo.opn_br_no, sAg_unitinfo.unit_no );
...
TrLog:
	...
	Ag_unitinfo_Clo_Sel();	
	ret = sql_execute( "insert into pay_agent_bk select * from 	pay_agent where tx_date = %ld and unit_no='1965'",g_pub_tx.tx_date );
	ret = sql_execute( "delete from pay_agent where tx_date = %ld and unit_no='1965'",g_pub_tx.tx_date );
OkExit:
