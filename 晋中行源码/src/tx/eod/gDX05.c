/*************************************************************
* 文 件 名: gDX05.c
* 功能描述：每月月底结息前3日即27日对欠息未还户发送短信提醒(不含按揭贷款)
*
* 作    者: zgf
* 完成日期: 2013年9月12日
*
...
insert into com_eod_exec values(9006,'7775','gDX05','月底欠息前3日提醒','1111111111','4','1','27','27','2','Y','Y','N','2','Y');
**************************************************************/
#include <stdio.h>
#include <fcntl.h>
#include <unistd.h>
...
{ 
	...
	while(1)
	{
		...
		while(1)
		{
			...
			/*pub_mob_sendmail_ln("贷款结息","7775",sMob_acct_type.ac_no,"1",ljqx,dqyh,dqqx);	*/
			pub_mob_sendmail_ln("贷款结息","7775",pact_no_tmp,"1",ljqx,dqyh,dqqx);
...
OkExit:
	...
	sprintf(acErrMsg, "欠息月底前3日对未还息户发送短信成功!!!!![%s]", g_pub_tx.reply);
	WRITEMSG
...
ErrExit:
	...
	sprintf(acErrMsg, "欠息月底前3日对未还息户发送短信失败!!!!![%s]", g_pub_tx.reply);
	WRITEMSG
		set_zd_data("0120", g_pub_tx.reply);
	return 1;
}
