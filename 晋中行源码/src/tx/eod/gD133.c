...
gD133()
{
	...
	if(iRc)
	{
        ...
        memset(sendbuf,0x00,sizeof(sendbuf));
        sprintf(sendbuf,"0022602900%ld%ld",com_sys_parm.lst_date,com_sys_parm.sys_date); /**设定交易代码为6029,标志位为00,报文体长度为22**/
	...
	if(i>=3)
	{
      vtcp_log("%s,%d,请求平台修改状态到日终是出错!i[%d],buf[%s]\n",__FILE__,__LINE__,i,buf);
			strcpy(g_pub_tx.reply, "9999");
			goto ErrExit;	
	}
	/******
	vtcp_log("%s,%d,进入等待时间!WYW_TEST",__FILE__,__LINE__);
	sleep(300);
	vtcp_log("%s,%d,等待时间过去!WYW_TEST",__FILE__,__LINE__);
	****/
	/* add by LiuHufeng 增加系统状态3表示真正翻盘 */
	iRc = sql_execute("update com_sys_parm set sys_sts = 3 ");
	if (iRc) {
