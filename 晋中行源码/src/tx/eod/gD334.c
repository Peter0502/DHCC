...
*
* sql: 
insert into com_eod_exec values('129','Z334','gD334','代理业务日终划款','1111111111','0','0','00','00','0','Y','Y','N','2','Y');
* 修改记录： 
* 日    期:
* 修 改 人:
* 修改内容:
...
gD334()
{
	...
	struct ag_ac_rel_c s_ag_ac_rel;
	...
	memset(&s_ag_ac_rel, 0x00, sizeof(struct ag_ac_rel_c));
	...
	strcpy(g_pub_tx.tx_code, "Z334");
	strcpy(g_pub_tx.sub_tx_code, "Z334");
	...
	vtcp_log("夜间划款开始");
	/*读表数据*/
	ret = Ag_ac_rel_Dec_Sel(g_pub_tx.reply,  "sts!='*' order by filler2 asc");
	  if ( ret )                                                                     
{                                                                              
	sprintf(acErrMsg,"查询关联账户表出错 [%d][%d]",ret,s_ag_ac_rel.main_ac_id);       
	WRITEMSG                                                                     
	   goto ErrExit;                                                                
}  
	...
	{
		memset(&s_ag_ac_rel, 0x00, sizeof(struct ag_ac_rel_c));
		ret = Ag_ac_rel_Fet_Sel(&s_ag_ac_rel, g_pub_tx.reply);
		if (ret == 100) break;
		MYSQLERR
	     vtcp_log("[&s_ag_ac_rel],s_ag_ac_rel[%ld]",s_ag_ac_rel.main_ac_id);      
		sprintf(main_ac_id, "%ld" ,s_ag_ac_rel.main_ac_id );
		if(main_ac_id[0] =='9')
		{
			ret=In_mst_Sel(g_pub_tx.reply,&in_mst_c,"ac_id='%ld' and sts!='*'", s_ag_ac_rel.main_ac_id);
			amt=in_mst_c.bal;
		...
		{
			ret=Dd_mst_Sel(g_pub_tx.reply,&dd_mst_a_c,"ac_id='%ld' and ac_sts!='*'", s_ag_ac_rel.main_ac_id);
			amt=dd_mst_a_c.bal;
	...
	/*begin  by guanjianfeng 增加手机充值划款 20150706*/
	if(strcmp(s_ag_ac_rel.filler2,"0601") == 0)/*划转话费手续费*/
	{
		if(amt>2500)
		{
			amt=amt-2500;
	...
	else if(strcmp(s_ag_ac_rel.filler2,"0602") == 0)/*划转流量手续费*/
	{
		if(amt>2500)
		{
			amt=amt-2500;
	...
	else if((strcmp(s_ag_ac_rel.filler2,"0611") == 0)||(strcmp(s_ag_ac_rel.filler2,"0612") == 0))
	{
		if(amt>2500)
		{
			amt=(amt-2500);
	...
	/*end  by guanjianfeng 增加手机充值划款 20150706*/
	...
	if(strcmp(s_ag_ac_rel.filler2,"0601")==0 || strcmp(s_ag_ac_rel.filler2,"0602")==0)
	{
	...
	{
		ret=Dd_mst_Sel(g_pub_tx.reply,&dd_mst_app,"ac_id='%ld' ",s_ag_ac_rel.app_ac_id);
		if ( ret )
		{
			sprintf(acErrMsg,"查询活期表出错 [%d][%d]",ret,s_ag_ac_rel.app_ac_id);
			WRITEMSG
			goto ErrExit;
		}
		memset(&sMdm_ac_rel_1,0x00,sizeof(struct mdm_ac_rel_c));
	...
	strcpy(g_pub_tx.cur_no,"01");
	g_pub_tx.ac_id=s_ag_ac_rel.app_ac_id;
	g_pub_tx.tx_amt1=amt;
	g_pub_tx.ac_seqn=1;
	strcpy( g_pub_tx.ac_id_type,"1" );
	strcpy(g_pub_tx.ac_get_ind,"00");
