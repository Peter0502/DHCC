...
gD407()
{
  ...
  memset(tx_date,0x00,sizeof(tx_date));
	...
	if(strncmp(tmp ,"15", 2)!=0 && strncmp(tmp ,"25", 2)!=0)
	{
		vtcp_log("不是15,25号 不代扣自来水费[%s]",tx_date);
		goto OkExit;
	
  }

	memset(filename,0x0,sizeof(filename));
	sprintf(filename, "%s/CDK%ld", getenv("FILDIR"), g_pub_tx.tx_date); 
	...
	while(1)
	{
		...
		vtcp_log(" [%d] [%s] ",0,fldstr[0]);
		for( i=0; i<30; i++ )
		{
			fldstr[1][i]=tmpstr[i+10];
			
		}
		pub_base_strpack( fldstr[1] );
		vtcp_log(" [%d] [%s] ",1,fldstr[1]);
		for( i=0; i<8; i++ )
		{
			fldstr[2][i]=tmpstr[i+40];
			
		}
		pub_base_strpack( fldstr[2] );
		vtcp_log(" [%d] [%s] ",2,fldstr[2]);
		for( i=0; i<8; i++ )
		{
			fldstr[3][i]=tmpstr[i+48];
			
		}
		pub_base_strpack( fldstr[3] );
		vtcp_log(" [%d] [%s] ",3,fldstr[3]);
		...
		if (strncmp(fldstr[3] ,tx_date,8) !=0 )
		{
		...
		ret = Ag_peritemmap_Sel( g_pub_tx.reply , &sAg_peritemmap,"unit_no = '%s' and upper(remark) = upper('%s') and stat='0'",cUnit_no, fldstr[0]);
		if(ret)
		{					
		...
		if(sMdm_ac_rel.note_sts[0] == '*')
		{
				...
				{
					strcpy( cReturn, "余额不足" );
					sprintf( acErrMsg, "余额不足[%s]", sAg_peritemmap.ac_no);
					WRITEMSG
					iProc_flag1 = 1;
					iErr_flag = 1;
