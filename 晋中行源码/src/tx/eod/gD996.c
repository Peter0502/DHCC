...
vtcp_log("begin create_boco_7 借据补充信息");
	ret=create_boco_7_1(g_pub_tx.tx_date);	/*** 借据补充信息 ***/
	if( ret )
		goto ErrExit;
...
vtcp_log("begin create_boco_27 主要业务状况日报表");
	ret=create_boco_27_1(g_pub_tx.tx_date);	/*** 主要业务状况日报表 ***/
	if( ret )
		goto ErrExit;
...
vtcp_log("filename=[%s]",filename);
	...
	/* 建临时索引 */
	ret = sql_execute("create index dd_mst_tmp on dd_mst(opn_date)");
	if ( ret )
	{
		sprintf(acErrMsg, "建立dd_mst的临时索引错误 [%d]", ret);
		WRITEMSG
		return ret;
	}

	ret=Dd_mst_Dec_Sel(g_pub_tx.reply," opn_date<=%d order by ac_id",g_pub_tx.tx_date);
	if( ret )	
	{ WRITEMSG return 1; }
	...
	while(1)
	{
		...
		vtcp_log("---dd_mst-----ac_id=[%ld]", g_dd_mst.ac_id);
	...
	ret = sql_execute("drop index dd_mst_tmp");
	if ( ret )
	{
		sprintf(acErrMsg, "删除dd_mst的临时索引错误 [%d]", ret);
		WRITEMSG
		return ret ;
	}

	/* 建临时索引 */
	ret = sql_execute("create index td_mst_tmp on td_mst(opn_date)");
	if ( ret )
	{
		sprintf(acErrMsg, "建立td_mst的临时索引错误 [%d]", ret);
		WRITEMSG
		return ret;
	}
	...
	while(1)
	{
		...
		vtcp_log("---td_mst-----ac_id=[%ld]", g_td_mst.ac_id);
	...
	ret = sql_execute("drop index td_mst_tmp");
	if ( ret )
	{
		sprintf(acErrMsg, "删除td_mst的临时索引错误 [%d]", ret);
		WRITEMSG
		return ret;
	}
...
int create_boco_3()
{
	...
	while(1)
	{
		...
		else
		{
			...
			fprintf(fp,"A|3%s|%s|%s|%s\n",
				g_dd_parm.prdt_no,g_dd_parm.title,g_dd_parm.cur_no,ac_type);
		}
		count++;
	}
	Dd_parm_Clo_Sel();
...
int create_boco_4()
{
	...
	/* 建临时索引 */
	ret = sql_execute("create index mdm_ac_rel_tmp on mdm_ac_rel(beg_date,mdm_code)");
	if ( ret )
	{
		sprintf(acErrMsg, "建立mdm_ac_rel的临时索引错误 [%d]", ret);
		WRITEMSG
		return ret;
	}

	ret=Mdm_ac_rel_Dec_Sel(g_pub_tx.reply," beg_date<=%d and mdm_code='0020' ",g_pub_tx.tx_date);
	if( ret )	
	{ WRITEMSG return 1; }
	...
	while(1)
	{
		...
		vtcp_log("---mdm_ac_rel-----ac_id=[%ld]", g_mdm_ac_rel.ac_id);
	...
	ret = sql_execute("drop index mdm_ac_rel_tmp");
	if ( ret )
	{
		sprintf(acErrMsg, "删除mdm_ac_rel的临时索引错误 [%d]", ret);
		WRITEMSG
		return ret;
	}
...
int create_boco_5()
{
	...
	FILE *fp;
	int	count=0;
	struct trace_log_rz_c	trace_log;
	...
	/* 建临时索引 */
/********************************XXX
	ret = sql_execute("create index trace_log_rz_tmp on trace_log_rz(sts,no_show)");
	if ( ret )
	{
		sprintf(acErrMsg, "建立mdm_ac_rel的临时索引错误 [%d]", ret);
		WRITEMSG
...
*****************************************/
	...
	ret=Trace_log_rz_Dec_Sel(g_pub_tx.reply," sts='0' and no_show!='1' ");
	if( ret )	
	{ WRITEMSG return 1; }
	...
	while(1)
	{
		...
		strcpy(jyfs,"3");		/*** 默认柜台 ***/
		strcpy(zjyw,"");
		ret=Trace_log_rz_Fet_Sel(&trace_log,g_pub_tx.reply);
		if( ret==100 ) break;
		if( ret )
		{ WRITEMSG return 1; }
		...
		if(!strcmp(trace_log.tx_code,"2203")||!strcmp(trace_log.tx_code,"2203"))
			strcpy(jylx,"5");			/*** 销户取款 ***/
...
/******************
	ret = sql_execute("drop index trace_log_rz_tmp");
	if ( ret )
	{
		sprintf(acErrMsg, "删除mdm_ac_rel的临时索引错误 [%d]", ret);
		WRITEMSG
...
*******************/
	...
	/* 虚拟流水(为了记录POS商户手续费) */
	ret=March_mst_hst_Dec_Sel(g_pub_tx.reply," deal_sign='Y' ");
	if( ret )	
	{ WRITEMSG return 1; }
...
int create_boco_6()
{
	...
	while(1)
	{
		...
		{
			ret=Ln_pay_pln_Sel(g_pub_tx.reply,&ln_pay_pln,"ac_id=%d and ac_seqn=0",g_ln_mst.ac_id);
			if( ret )
			{ WRITEMSG return 1; }
	...
	/* 建临时索引 */
	ret = sql_execute("create index In_mst_tmp on In_mst(sts,opn_date)");
	if ( ret )
	{
		sprintf(acErrMsg, "建立In_mst的临时索引错误 [%d]", ret);
		WRITEMSG
		return ret;
	}
	...
	while(1)
	{
		...
		/*** 取贴现登记薄 ***/
		memset(&mo_discnt, 0x0, sizeof(mo_discnt));
		...
		/*** 取会计科目 ***/
		ret=Dc_acc_rel_Sel(g_pub_tx.reply,&dc_acc_rel,"dc_code='%s' and data_code='0152'",g_in_parm.dc_code);
	...
	ret = sql_execute("drop index In_mst_tmp");
	if ( ret )
	{
		sprintf(acErrMsg, "删除In_mst的临时索引错误 [%d]", ret);
		WRITEMSG
		return ret;
	}
...
int create_boco_9()
{
	...
	/* 建临时索引 */
	ret = sql_execute("create index Ln_reg_tmp on Ln_reg(reg_date_beg)");
	if ( ret )
	{
		sprintf(acErrMsg, "建立Ln_reg的临时索引错误 [%d]", ret);
		WRITEMSG
		return ret;
	}
	...
	ret = sql_execute("drop index Ln_reg_tmp");
	if ( ret )
	{
		sprintf(acErrMsg, "删除Ln_reg的临时索引错误 [%d]", ret);
		WRITEMSG
		return ret;
	}

	/* 建临时索引 */
	ret = sql_execute("create index Ln_mst_tmp on Ln_mst(opn_date,ac_sts,intst_type)");
	if ( ret )
	{
		sprintf(acErrMsg, "建立Ln_mst的临时索引错误 [%d]", ret);
		WRITEMSG
		return ret;
	}
	...
	ret = sql_execute("drop index Ln_mst_tmp");
	if ( ret )
	{
		sprintf(acErrMsg, "删除Ln_mst的临时索引错误 [%d]", ret);
		WRITEMSG
		return ret;
	}

	/* 建临时索引 */
	ret = sql_execute("create index trace_log_rz_tmp on trace_log_rz(tx_code)");
	if ( ret )
	{
		sprintf(acErrMsg, "建立trace_log_rz的临时索引错误 [%d]", ret);
		WRITEMSG
		return ret;
	}

	/*** 从流水中取本金、利息减少的贷款 ***/
	ret=Trace_log_rz_Dec_Sel(g_pub_tx.reply," tx_code='L201' or tx_code='L202' or tx_code='L203' or tx_code='L204' order by trace_no");
	if( ret )
	{ WRITEMSG return 1; }
	...
	if ( ret )
	{
		sprintf(acErrMsg, "删除trace_log_rz的临时索引错误 [%d]", ret);
		WRITEMSG
		return ret;
	}
...
int create_boco_10()
{
	...
	/* 建临时索引 */
	ret = sql_execute("create index mbfebillopnreg_tmp on mbfebillopnreg(consigndate)");
	if ( ret )
	{
		sprintf(acErrMsg, "建立mbfebillopnreg的临时索引错误 [%d]", ret);
		WRITEMSG
		return ret;
	}
	...
	*******/
	...
	ret = sql_execute("drop index mbfebillopnreg_tmp");
	if ( ret )
	{
		sprintf(acErrMsg, "删除mbfebillopnreg_tmp的临时索引错误 [%d]", ret);
		WRITEMSG
		return ret;
	}
...
int create_boco_11()
{
	...
	/* 建临时索引 */
	ret = sql_execute("create index trace_log_rz_tmp on trace_log_rz(sub_tx_code,sts, ac_no)");
	if ( ret )
	{
		sprintf(acErrMsg, "建立trace_log_rz的临时索引错误 [%d]", ret);
		WRITEMSG
	...
	/*** 承兑汇票 ***/
	ret=Trace_log_rz_Dec_Sel(g_pub_tx.reply," (sub_tx_code='B334' or sub_tx_code='B337') and sts='0' and ac_no[1,3]!='702' ");	
	if( ret )
	{ WRITEMSG return 1; }
	...
	if ( ret )
	{
		sprintf(acErrMsg, "删除trace_log_rz_tmp的临时索引错误 [%d]", ret);
		WRITEMSG
		return ret;
	}
...
int create_boco_24()
{
	...
	/* 建临时索引 */
	ret = sql_execute("create index trace_log_rz_tmp on trace_log_rz(tx_code,sts)");
	if ( ret )
	{
		sprintf(acErrMsg, "建立trace_log_rz的临时索引错误 [%d]", ret);
		WRITEMSG
	...
	ret=Trace_log_rz_Dec_Sel(g_pub_tx.reply," tx_code='4703' and sts='0' order by trace_no,sub_tx_code ");
	if( ret )	
	{ WRITEMSG return 1; }
...
int create_boco_25()
{
	...
	/* 建临时索引 */
	ret = sql_execute("create index trace_log_rz_tmp on trace_log_rz(tx_code,ac_id,sts)");
	if ( ret )
	{
		sprintf(acErrMsg, "建立trace_log_rz的临时索引错误 [%d]", ret);
		WRITEMSG
	...
	ret=Trace_log_rz_Dec_Sel(g_pub_tx.reply," tx_code='4705' and ac_id!=0 and sts='0' order by trace_no,sub_tx_code ");
	if( ret )	
	{ WRITEMSG return 1; }
...
int create_boco_26()
{
	...
	/* 建临时索引 */
	ret = sql_execute("create index trace_log_rz_tmp on trace_log_rz(amt,sts,no_show)");
	if ( ret )
	{
		sprintf(acErrMsg, "建立trace_log_rz的临时索引错误 [%d]", ret);
		WRITEMSG
	...
	ret=Trace_log_rz_Dec_Sel(g_pub_tx.reply," tx_code not in('5504','5202','5203') and  amt>500000.00 and sts='0' and no_show!='1' order by trace_no,sub_tx_code ");
	if( ret )	
	{ WRITEMSG return 1; }
...
int create_boco_28()
{
	...
	ret=Cif_basic_inf_Dec_Sel( g_pub_tx.reply," sts in ('1','0')" );
	if( ret )	
	{ WRITEMSG return 1; }
...
int create_boco_29()
{
	...
	ret=Cif_rel_chg_hst_Dec_Sel( g_pub_tx.reply,"chg_tablename='cif_basic_inf' and chg_columnname='name'  ");
	if( ret )	
	{ WRITEMSG return 1; }
