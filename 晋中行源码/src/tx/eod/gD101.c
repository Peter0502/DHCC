...
gD101()
{
	...
	while (1)
	{
		...
		ret = Dd_mst ( g_pub_tx.reply, &sDd_mst, "ac_id = %ld \
			and ac_seqn = 1", sMdm_ac_rel.ac_id );
		if ( ret )
		{
			sprintf( acErrMsg, "查询[%s]活期主文件出错", sMob_acct_type.ac_no );
			WRITEMSG
			goto ErrExit;
		}
		...
		sprintf( acErrMsg, "尊敬的客户:您的账(卡)%s本日末活期余额为%.2f元。商业银行", sMob_acct_type.ac_no, sDd_mst.bal );
...
OkExit:
	...
	sprintf(acErrMsg,"短信统计当日活期余额成功![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 0;
ErrExit:
	sql_rollback();	/*--事务回滚--*/
	sprintf(acErrMsg,"短信统计当日活期余额失败![%s]",g_pub_tx.reply);
	WRITEMSG
	set_zd_data("0120",g_pub_tx.reply);
	return 1;
}
