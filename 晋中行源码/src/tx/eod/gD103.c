...
gD103()
{
	...
	while (1)
	{
		...
		ret = sql_sum_double ( "dd_mst_hst" , "tx_amt" , &dJfs , \
			"ac_id = %ld and ac_seqn = 1 and tx_date = %ld \
			and add_ind = '0'", \
			sMdm_ac_rel.ac_id, g_pub_tx.tx_date );
		if ( ret != 0 )
		{
		...
		ret = sql_sum_double ( "dd_mst_hst" , "tx_amt" , &dDfs , \
			"ac_id = 1 and ac_seqn = %d and tx_date = %ld \
			and add_ind = '1'", \
			sMdm_ac_rel.ac_id, g_pub_tx.tx_date );
		if ( ret != 0 )
		{
		...
		sprintf( acErrMsg, "尊敬的客户:您的活期账户%s当天累计取款%.2f元存款%.2f元.商业银行", dJfs, dDfs );
