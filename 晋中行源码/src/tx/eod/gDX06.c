/*************************************************************
* 文 件 名: gDX06.c
* 功能描述：结息日前3日即17日对贷款户发送短信提醒(不含按揭贷款)
*
* 作    者: zgf
* 完成日期: 2013年9月12日
*
...
insert into com_eod_exec values(9005,'7776','gDX06','贷款结息前3日提醒','1111111111','4','1','17','17','2','Y','Y','N','2','Y');
**************************************************************/
#include <stdio.h>
#include <fcntl.h>
#include <unistd.h>
...
/*
			...
			g_reply_int=Ln_mst_Dec_Sel2(g_pub_tx.reply,"repay_ac_id=%ld and ac_sts<>'*' and bal > 0 and intst_type in('3') ",sMob_acct_type.ac_id);
		}
		if(g_reply_int)
		{
		...
		while(1)
		{
			...
			pub_mob_sendmail_ln("贷款结息","7776",pact_no_tmp,"1",ljqx,dqyh,dqqx);	
...
OkExit:
	...
	sprintf(acErrMsg, "结息前3日对还息户发送短信成功!!!!![%s]", g_pub_tx.reply);
	WRITEMSG
...
ErrExit:
	...
	sprintf(acErrMsg, "结息前3日对还息户发送短信失败!!!!![%s]", g_pub_tx.reply);
	WRITEMSG
		set_zd_data("0120", g_pub_tx.reply);
	return 1;
}
