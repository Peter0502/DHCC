...
gD067()
{
    ...
    /* 统计金额 */
	/* 提出借方 */
    ret = sql_sum_double("mo_city","bill_amt",&amt1,"in_out_ind='1' and \
			cd_ind='1' and (sts='1' or sts='6')");
    if (ret != 0)
    {
        sprintf(acErrMsg,"统计同城提出挂账金额错误 [%d]",ret);
        WRITEMSG
        goto ErrExit;
    }

    sprintf(acErrMsg,"提出借方金额=[%lf]",amt1);
    WRITEMSG

	/* 提出贷方 */
    ret = sql_sum_double("mo_city","bill_amt",&amt2,"in_out_ind='1' and \
			cd_ind='2' and (sts='1' or sts='6')");
    if (ret != 0)
    {
        sprintf(acErrMsg,"统计同城提出挂账金额错误 [%d]",ret);
        WRITEMSG
    ...
    sprintf(acErrMsg,"提出贷方金额=[%lf]",amt2);
    WRITEMSG
...
OkExit:
	...
	sprintf(acErrMsg,"中心提出销帐(日终)成功![%s]",g_pub_tx.reply);
    WRITEMSG
...
ErrExit:
    ...
    sql_rollback(); /*--事务回滚--*/
	sprintf(acErrMsg,"中心提出销帐(日终)失败![%s]",g_pub_tx.reply);
    WRITEMSG
    set_zd_data("0120",g_pub_tx.reply);
    return 1;
}
