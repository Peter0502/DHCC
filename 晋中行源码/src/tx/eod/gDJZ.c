...
gDJZ()
{
		...
		ret=New_jz_prdt_Dec_Upd(g_pub_tx.reply," sts='0' and 1=1 order by pact_no");
		if(ret)
		{
		...
		while(1)
		{
			...
			}else if(ret)
			{
					...
					{
										vtcp_log("%s,%ld核心不存在该借据号%s",__FILE__,__LINE__,sNew_jz_prdt.pact_no);	
										sNew_jz_prdt.sts[0]='3';
										ret=New_jz_prdt_Upd_Upd(sNew_jz_prdt,g_pub_tx.reply);
										if(ret)
										{
											sprintf( acErrMsg, "执行sNew_jz_prdt_Upd_Upd出错!ret=[%d]",ret );
													WRITEMSG
					...
					
					{
						...
						{
									ret=bj_jzzc(cAc_no,sNew_jz_prdt.old_prdt_no,sNew_jz_prdt.new_prdt_no);
									if(ret&&ret!=2)
									{
									...
									ret=bnlx_jzzc(cAc_no,sNew_jz_prdt.old_prdt_no,sNew_jz_prdt.new_prdt_no);
									if(ret&&ret!=2)
									{
										sprintf( acErrMsg, "利息科目记账转出出错!ret=[%d]",ret );
											WRITEMSG
									...
									ret=bwlx_jzzc(cAc_no,sNew_jz_prdt.old_prdt_no,sNew_jz_prdt.new_prdt_no);
									if(ret&&ret!=2)
									{
										sprintf( acErrMsg, "利息科目记账转出出错!ret=[%d]",ret );
											WRITEMSG
									...
									ret=bj_jzzr(cAc_no,sNew_jz_prdt.old_prdt_no,sNew_jz_prdt.new_prdt_no);
									if(ret&&ret!=2)
									{
										sprintf( acErrMsg, "利息科目记账转出出错!ret=[%d]",ret );
											WRITEMSG
									...
									ret=bnlx_jzzr(cAc_no,sNew_jz_prdt.old_prdt_no,sNew_jz_prdt.new_prdt_no);
									if(ret&&ret!=2)
									{
									...
									ret=bwlx_jzzr(cAc_no,sNew_jz_prdt.old_prdt_no,sNew_jz_prdt.new_prdt_no);
									if(ret&&ret!=2)
									{
										sprintf( acErrMsg, "利息科目记账转入出错!ret=[%d]",ret );
											WRITEMSG
						...
						{
										vtcp_log("产品未进行减值转换，请确认是否为减值产品");
										TRACE
										sprintf( acErrMsg, "产品未进行减值转换，请确认是否为减值产品!ac_no=[%s]",sMdm_ac_rel.ac_no );
											WRITEMSG
											continue;
					...
					strcpy(sNew_jz_prdt.tel,"9999");
					strcpy(sNew_jz_prdt.tx_br_no,"19999");
					sNew_jz_prdt.sts[0]='1';
					ret=New_jz_prdt_Upd_Upd(sNew_jz_prdt,g_pub_tx.reply);
					if(ret)
					{
...
int bj_jzzr(char *ac_no,char *old_prod_no,char *new_prod_no)
{
	...
	strcpy(sTrace_log.tx_code,"gDJZ");
	strcpy(sTrace_log.sub_tx_code,"gDJZ");
