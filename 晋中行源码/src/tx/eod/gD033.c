...
/*    ret = Eod_Dd_mst_Dec_Upd(g_pub_tx.reply,
		"ac_type!='5' and ac_type!='7' and ac_sts='1' and prdt_no not in ('131','142') ");	 保证金不结息 modify by chenchao 2011/9/1 22:27:53*/
	/*ret = Eod_Dd_mst_Dec_Upd(g_pub_tx.reply,
		"ac_type!='5' and ac_type!='7' and ac_sts='1' and prdt_no!='131'  "); 20130317 wjl   添加七天乐虚拟户产品107 不计息*/
	ret = Eod_Dd_mst_Dec_Upd(g_pub_tx.reply,
		"ac_type!='5' and ac_type!='7' and ac_sts='1' and prdt_no!='131'and  prdt_no!='107'  "); 	
	MYSQLERR
    ...
    while(1)
    {
		...
		/*0：不计息；1：利随本清；2：按日计息；*/
		if( dd_mst_c.intst_type[0]=='0' || dd_mst_c.intst_type[0]=='1' 
		...
		while(1)
		{
        	...
        	intst_hst_c.intst_tax = pub_base_PubDround_1(com_sect_acm_c.intst_acm*com_sect_acm_c.rate/30)*0.2;
                ***/
                intst_hst_c.fct_intst = pub_base_PubDround_1(com_sect_acm_c.intst_acm * com_sect_acm_c.rate/L360/L100);
                intst_hst_c.intst_tax = 0; 
        	intst_hst_c.aic_bal = dd_mst_c.bal + intst_hst_c.fct_intst-intst_hst_c.intst_tax;
 	       	/*** intst_hst_c.aic_acm = dd_mst_c.intst_acm;      20110407分段积数表放年利率***/
                intst_hst_c.aic_acm = com_sect_acm_c.intst_acm;
        ...
        strcpy(intst_hst_c.ac_ind,"0");
        strcpy(intst_hst_c.pt_ind,"0");
	if(strncmp(dd_mst_c.prdt_no,"142",sizeof(dd_mst_c.prdt_no)-1) == 0 || dd_mst_c.ac_type[0]=='6')/**  担保保证金取分户利率,协定存款户取分户利率**/
	{
