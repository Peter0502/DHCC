...
int spF004()
{
	...
	memset(cTx_amt, 0x0,sizeof(cTx_amt));
	if (pub_base_sysinit())
	{
	...
	sprintf(acErrMsg,"取款金额=[%lf]", g_pub_tx.tx_amt1);
	WRITEMSG
	...
	********/
	...
	get_zd_long("0450", &chnl_self_trade.dz_date);  /* 前台交易日期*/
	...
	get_zd_data("102K", chnl_self_trade.ct_ind);
