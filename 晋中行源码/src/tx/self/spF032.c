/*************************************************
* 文 件 名:  spF032.c
* 功能描述： POS 预授权完成(转账到商户的帐户上)(帐号是本行的商户)
*				
* 作    者:  rob
* 完成日期： 2003年2月26日
*
...
*   日   期:
*   修 改 人:
*   修改内容: 这个交易虽然配在预授权交易流程里了,但是不能走,除非隔日
*             有对于本行商户扣除手续费的交易,并且还要配置此子交易的dc_entry
*************************************************/
...
int spF032()
{
	...
	if(strlen(chnl_marchinfo.mrchno)==0)
	{
		strcpy(chnl_marchinfo.mrchno, "test");
	}
    ...
    /*  如果是预授权完成的话,需要根据预授权号和其他查询预授权交易
       修改其状态为非正常状态   */
    ret = Chnl_self_trade_Dec_Upd(g_pub_tx.reply,"premise_no=%ld and brf_no='PSSQ'", 
          g_pub_tx.auth_code);
    if (ret != 0)
    {
