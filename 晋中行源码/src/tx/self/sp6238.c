...
insert into tx_def  values ('6238', '联机单笔转账记账', '10000000000000000000000000000100000000010000000000000000000001000101111000000000100000000000000000000000000000000000000000000000', '1', '1', '0', '3', null);
...
insert into tx_flow_def  values ('6238', 7, 'F000', '#$');
...
int sp6238()
{
    ...
    **/
	...
	/*** TODO:如果核心已经记账成功,直接返回 ***/
	memset(&sChnl_single_acct, 0x0, sizeof(sChnl_single_acct));
	iRet = Chnl_single_acct_Sel(g_pub_tx.reply, &sChnl_single_acct, "pt_date = %d and pt_trace_no = %d and bbmk = '%s' and sts = '1'", 
			lPt_date, lPt_trace_no, cHx_term);
	if (iRet == 0)
	{
		vtcp_log("[%s][%d] 平台日期:[%d] 平台流水:[%d] BBMK=[%s] 已记账成功,直接返回!",
			__FILE__, __LINE__, lPt_date, lPt_trace_no, cHx_term);
		set_zd_long("0040", sChnl_single_acct.trace_no);
		set_zd_data("0060", sChnl_single_acct.tx_time);
		set_zd_long("0460", sChnl_single_acct.tx_date);
		strcpy(g_pub_tx.reply, "000A");
		vtcp_log("[%s][%d] 核心日期=[%d] 核心流水=[%d] 核心时间=[%s]",
			__FILE__, __LINE__, sChnl_single_acct.tx_date, sChnl_single_acct.trace_no, sChnl_single_acct.tx_time);
		goto ErrExit;
	}
	...
	(memcmp(cHx_term,"sxps",4) == 0  && memcmp(cYw_type,"1000",4) ==0)
      )
    {
	...
	else if (memcmp(cHx_term,"sxps",4) == 0)
	{
		iParm_seqn = 30;
	}
        else
    ...
    if( strlen ( cAc_no1 ) >10) /*** 此种情况下,只能是账号 ***/
    {
        ...
        if(sPrdt_ac_id.ac_id_type[0] == '1')
        {
            ...
            if(cDc_ind[0] == '1')   /*1-借*/
            {
                ...
                /*6238不检查余额 fupengfei 2014/7/10 15:46:26*/
        ...
        vtcp_log("[%s],[%d],入账机构，opn_br_no=[%s],sRz_br_no=[%s]!",__FILE__,__LINE__,g_mdm_ac_rel.opn_br_no,sRz_br_no);
    ...
    memcpy(sChnl_single_acct.opn_br_no,g_pub_tx.opn_br_no,sizeof(sChnl_single_acct.opn_br_no)-1);    /** 开户机构 **/
