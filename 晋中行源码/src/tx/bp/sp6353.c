...
int sp6353()
{
    ...
    memset(&olv_pkgreg , 0 , sizeof(olv_pkgreg));
    
    vtcp_log("sp6353 开始！[%s][%d]",__FILE__,__LINE__);
    ...
    /******获取三个索引的值*******/
    get_zd_data("1280",olv_pkgreg.orderno);
    get_zd_long("0440",&olv_pkgreg.wt_date);
    GetOr_br_noByBr_no(cKinbr, olv_pkgreg.or_br_no);

    vtcp_log("[%s][%d]根据这里锁定小额支付表:wt_date=[%ld],or_br_no=[%s],orderno=[%s]"\
    ,__FILE__,__LINE__,olv_pkgreg.wt_date,olv_pkgreg.or_br_no,olv_pkgreg.orderno);
    
    vtcp_log("sp6353 开始！[%s][%d]",__FILE__,__LINE__);

    ret = Lv_pkgreg_Sel(g_pub_tx.reply,&olv_pkgreg, "orderno='%s' and or_br_no='%s' \
    and wt_date=%ld and pkgno='004' and lw_ind='1' ", olv_pkgreg.orderno,olv_pkgreg.or_br_no,olv_pkgreg.wt_date,cKinbr);
    ...
    if(olv_pkgreg.lv_sts[0] !='5')
    {
        sprintf(acErrMsg,"原业务非拒绝状态");
        vtcp_log("%s,%d 原业务非拒绝状态",__FILE__,__LINE__,cKinbr);
        set_zd_data(DC_GET_MSG,acErrMsg);
        strcpy(g_pub_tx.reply,"8888");
        WRITEMSG
        goto ErrExit;
    }
    ret = Lv_pkgreg_Sel(g_pub_tx.reply,&lv_pkgreg, "o_orderno='%s' and o_or_br_no='%s' \
    and o_wt_date=%ld and pkgno='010' and lw_ind='2' ", olv_pkgreg.orderno,olv_pkgreg.or_br_no,olv_pkgreg.wt_date,cKinbr);
    vtcp_log("[%s][%d]",__FILE__,__LINE__);
    if ( ret==100 )
    {
        sprintf(acErrMsg,"没有收到该业务的回执信息!!");
        set_zd_data(DC_GET_MSG,acErrMsg);
        strcpy( g_pub_tx.reply, "D104" );
        WRITEMSG
        goto    ErrExit;
