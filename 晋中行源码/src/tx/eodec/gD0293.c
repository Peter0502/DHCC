		...
		while(1)
		{
      	...
      	set_zd_double("1218",0.00 );
			if(sGl_sub.cr_bal!=0)
			{
				...
				g_pub_tx.tx_amt1=-sGl_sub.cr_bal;/*支行贷方负*/
				vtcp_log(acErrMsg, "%s,%d aaaaa[%lf]----------",__FILE__,__LINE__,g_pub_tx.tx_amt1);
				g_pub_tx.ct_ind[0]='2';
      	...
      	set_zd_double("1218",0.00 );
				...
				vtcp_log(acErrMsg, "%s,%d ----------",__FILE__,__LINE__);
				sprintf(acErrMsg, "同时处理[%s]到清算科目[%s]---------->",sCom_branch.br_no,sGl_sub.acc_hrt);
				WRITEMSG
				set_zd_data("1211",sCom_item.acc_no);
				set_zd_double("1218",g_pub_tx.tx_amt1);
				set_zd_data("121A",g_pub_tx.brf);
