drop index lv_pkgreg_idx;
drop index lv_pkgreg_idx2;
drop index lv_pkgreg_idx3;
drop table lv_pkgreg;
create table lv_pkgreg(
  ...
  lv_o_sts        varchar2(1) ,/*原始来帐交易状态退汇交易用,信息类借用为附加文类型0－内容1－文件*/  
  lw_ind          varchar2(1) ,/*来往标识 1往2来   */
  cur_no          varchar2(3) ,/*币种                   */
  filler          varchar2(60)/*备注              */
)tablespace users;
create unique index lv_pkgreg_idx on lv_pkgreg(or_br_no,pkgno,wt_date,orderno,lw_ind)  tablespace indx;
create index lv_pkgreg_idx2 on lv_pkgreg (packid,pack_date,br_no,pay_qs_no,lw_ind);
create index lv_pkgreg_idx3 on lv_pkgreg (in_date,br_no,lw_ind);