      /**	往包控制表 **/
drop index lv_wbctl_idx;
drop index lv_wbctl_idx2;
drop table lv_wbctl;
create table lv_wbctl(
   	pack_date	Varchar2(8)	, /**  打包日期			**/
	packid		Varchar2(8)	, /** 包id				**/
	pkgno		Varchar2(3)	, /** Pkg编号			**/
	pay_qs_no	Varchar2(12), /** 发起清算行号		**/
	cash_qs_no	Varchar2(12), /** 接收清算行号		**/
	in_date		Varchar2(8)	, /** 进入日期			**/
	packmy		Varchar2(40), /** 包密押				**/
	dtlcnt		Number(8)	, /** 明细业务总笔数		**/
	totamt		Number(24,2), /** 明细业务总金额		**/
	zcnode		Varchar2(1)	, /** 轧差节点类型		**/
	zc_date		Varchar2(8)	, /** 轧差日期			**/
	zcid		Varchar2(2)	, /** 轧差场次			**/
	hz_date		Varchar2(8)	, /** 回执期限（日期）	**/
	o_pack_date	Varchar2(8)	, /** 原来包日期			**/
	o_packid	Varchar2(8)	, /** 原来包id			**/
	dtlsuccnt	Number(8)	, /** 明细业务成功笔数	**/
...
create unique index lv_wbctl_idx on lv_wbctl(pack_date,packid)tablespace indx;

create index lv_wbctl_idx2 on lv_wbctl(o_pack_date,o_packid,br_no,in_date)tablespace indx;
