...
int makedb_tab(char *pcTabName)
{
	...
	fprintf(fp,"	va_end(ap);\n");
	fprintf(fp,"\n");
	fprintf(fp,"	sprintf ( comm ,\" SELECT %s.* FROM %s WHERE  \");\n",pcTabName,pcTabName);
	...
	fprintf(fp,"\n");
	fprintf(fp,"	EXEC SQL DELETE FROM %s WHERE rowid='0000';\n",pcTabName,pcTabName);
	fprintf(fp,"	if ( sqlca.sqlcode ){\n");
...
int main(){
	...
	FILE *fp;
	char line[1024];
	sprintf(pcFileName,"%s/etc/table_name_notab",getenv("DBSDIR"),TABNDIR);
