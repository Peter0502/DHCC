	                             ...
	                             char *tagName,char *data)
{
	...
	if(pTag !=NULL)
	{
		...
		{
			vtcp_log("tagName和beforeTagName不在相临位置\n");
			return -1;
		}else{
...
int hv_analyze_pack(THVPACKAGE *hv)
{
	...
	if (memcmp(hv->body+CMTCODE,"689",3)==0)
	{
		...
		{
			vtcp_log("[%s][%d]检查到错误的报文结构，报文体不合法。\n",__FILE__,__LINE__);
			return -1;
		}
		...
		if(hv->body[hv->length-1] != '}')
		{
			vtcp_log("检查到错误的报文结构(不以\"}\"结尾)，报文体不合法。\n");
			return -1;
		}
	...
	while(1)
	{
		...
		{
			vtcp_log("检查到错误的tagName长度，这个报文体不合法。\n");
			return -1;
		}
...
{
	iGetHvFlgno(hv->reNo,hv->flgNo);
}
void getPackageReNo(THVPACKAGE *hv)
{
	/*****参考号在交易中赋值
	iGetHvRefno(HV_BANKCODE_FLGNO,hv->reNo);
	*****/
}
