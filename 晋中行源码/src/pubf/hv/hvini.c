...
int  fgetline(FILE *fp, char *buffer, int maxlen)
{
	...
	MEMSET_DEBUG(buffer, 0x00, maxlen);
		...
		if(n>1)
		{
			...
			if(isItem == 0)
			{
				...
				if(buf[valP] == '=')
				{
					...
					MEMCPY_DEBUG(val,buf+valP,n-valP);
					val[n-valP]='\0';
					return line_no;
				}
...
int  writeLine(char *filename,char *buf,int line_no)
{
	...
	MEMCPY_DEBUG(tempfile,filename,strlen(filename));
	sprintf(tempfile,"%s.%d.temp",filename,getpid);
	MEMCPY_DEBUG(tempfile+strlen(filename),".temp",5);
	...
	while(1)
	{
		...
		{
			fprintf(tempfp,"%s\n",val);
			MEMCPY_DEBUG(val,buf,strlen(buf));
		}
		fprintf(tempfp,"%s\n",val);
	}
	fclose(fp);
