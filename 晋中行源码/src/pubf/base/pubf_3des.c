...
static void D2des(unsigned char *, unsigned char *);
...
{
	{
		char sKey[]="\x01\x23\x45\x67\x89\xab\xcd\xef\xfe\xdc\xba\x98\x76\x54\x32\x10";
		char sPlain[]="\x01\x23\x45\x67\x89\xab\xcd\xe7";
		char sCipher[16];
		memset(sCipher,'\0',sizeof(sCipher));
		des2key(sKey,0);
		Ddes(sPlain,sCipher);
		printf("加密测试:\n");
		printf("案例密文:7f1d 0a77 826b 8aff\n");
		printf("计算密文:%02x%02x %02x%02x %02x%02x %02x%02x\n",sCipher[0],sCipher[1],
				sCipher[2],sCipher[3],sCipher[4],sCipher[5],sCipher[6],sCipher[7]);	
	}
	printf("============================\n");
	{
		char sKey[]="\x01\x23\x45\x67\x89\xab\xcd\xef\xfe\xdc\xba\x98\x76\x54\x32\x10";
		char sPlain[16];
		char sCipher[]="\x7f\x1d\x0a\x77\x82\x6b\x8a\xff";
		memset(sPlain,'\0',sizeof(sPlain));
		des2key(sKey,1);
		Ddes(sCipher,sPlain);
		printf("解密测试:\n");
		printf("案例明文:0123 4567 89ab cde7\n");
		printf("计算明文:%02x%02x %02x%02x %02x%02x %02x%02x\n",sPlain[0],sPlain[1],
				sPlain[2],sPlain[3],sPlain[4],sPlain[5],sPlain[6],sPlain[7]);	
	}
	return(0);
}
****/
/************************************************************************
 * 函 数 名：pubf_3des
 * 函数功能：用密钥key_str对from_str进行3des算法处理，结果在into_str中。
 * 参    数：mode 0加密 1解密；key_len是key_str长度；from_len是from_str长度
 * 返 回 值：0 成功 -1失败
 ************************************************************************/
static int pubf_3des(short mode, unsigned char *key_str, int key_len, unsigned char *from_str, unsigned char *into_str, int from_len)
{
	if (8 == key_len && 8 == from_len) 
	{
		deskey(key_str, mode);
		des(from_str, into_str);
	}
	else if (16 == key_len && 8 == from_len) 
	{
		des2key(key_str, mode);
		Ddes(from_str, into_str);
	}
	else if (16 == key_len && 16 == from_len) 
	{
		des2key(key_str, mode);
		Ddes(from_str, into_str);
		Ddes(from_str + 8, into_str + 8);
	}
	else 
	{
		/*
                uLog(LOG_ERR,"%s()key_len[%d]str_len[%d]错误",__func__,key_len,from_len);
                */
                vtcp_log("[%s][%d],%s()key_len[%d]str_len[%d]错误",__FILE__,__LINE__,__func__,key_len,from_len);
		return -1;
	}
	return 0;
}
...
{
	if ( pubf_3des(0, key_str, key_len, from_str, into_str, from_len) ) 
	{
		return -1;
	}
	return 0;
...
{
	if ( pubf_3des(1, key_str, key_len, from_str, into_str, from_len) ) 
	{
		return -1;
	}
	return 0;
